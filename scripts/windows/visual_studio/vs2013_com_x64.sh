# Visual Studio 2013 Community (64-bit) setup script for Cygwin.
#
# Insert this into a Cygwin startup like $HOME/.bashrc like this
#
# if [ x$FrameworkDir = x ]; then
#     if [ -f /usr/local/bin/vs2013_com_x64.sh ]; then
#         source /usr/local/bin/vs2013_com_x64.sh
#     fi
# fi
#
...
# of Visual Studio 2013.  They are current as of 2016-01-20
# and include the changes of Update 4.
#
...
VISUAL_STUDIO_DIR="$PROGRAM_FILES_DIR\\Microsoft Visual Studio 12.0"

KIT_DIR="$PROGRAM_FILES_DIR\\Windows Kits"

SDK_DIR="$PROGRAM_FILES_DIR\\Microsoft SDKs\\Windows\\v8.1"

DOTNET_DIR="C:\\WINDOWS\\Microsoft.NET"

export CommandPromptType=Native

export ExtensionSdkDir="$SDK_DIR\\ExtensionSDKs"
...
export LIB="$LIB;$KIT_DIR\\8.1\\lib\\winv6.3\\um\\x64;"
       ...
       LIBPATH="$LIBPATH;$VISUAL_STUDIO_DIR\\VC\\ATLMFC\\LIB\\amd64"
       LIBPATH="$LIBPATH;$KIT_DIR\\8.1\\References\\CommonConfiguration\\Neutral"
export LIBPATH="$LIBPATH;$SDK_DIR\\ExtensionSDKs\\Microsoft.VClibs\\12.0\\References\\CommonConfiguration\\neutral"
...
export WindowsSDK_ExecutablePath_x64="${SDK_DIR}A\\bin\\NETFX $NETFX_VERSION Tools\\x64\\"
export WindowsSDK_ExecutablePath_x86="${SDK_DIR}A\\bin\\NETFX $NETFX_VERSION Tools\\"
...
tmp=`cygpath -d -m "${SDK_DIR}A\\bin\\NETFX $NETFX_VERSION Tools"`
...
new_path="$new_path:$cyg_program_files_dir/MSBuild/12.0/bin/amd64"
