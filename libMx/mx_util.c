...
mx_is_valid_pointer( void *pointer, size_t length, int access_mode )
{
	...
	if ( file == NULL ) {
		/* If opening /proc/self/maps fails, then we attempt to
		 * use msync() to find out the necessary information.
		 * This uses a _hack_ that is described here:
		 *
		 * http://groups.google.com/group/
		 * comp.os.linux.development.apps/msg/acc1590469aebb22
		 *
		 * FIXME: This is not an ideal way of getting
		 * the information.
		 *
		 */

		os_status = msync( pointer, length, MS_ASYNC );
		...
		if ( os_status == 0 ) {
			...
			if ( saved_errno == ENOMEM ) {
				...
				(void) mx_error(
				MXE_OPERATING_SYSTEM_ERROR, fname,
				"The attempt to invoke msync() for the "
				"pointer %p failed with errno = %d.  "
	...
	for ( i = 0; ; i++ ) {
		...
		MX_DEBUG(-2,("%s: argc = %d, argv[0] = '%s', "
			"argv[1] = '%s', argv[2] = '%s'",
			fname, argc, argv[0], argv[1], argv[2] ));
