...
#define FREE_MOTOR_POSITION_ARRAYS \
		mxs_energy_mcs_quick_scan_free_arrays(scan,quick_scan,energy_mcs_quick_scan)

static void
mxs_energy_mcs_quick_scan_free_arrays( MX_SCAN *scan,
				MX_QUICK_SCAN *quick_scan,
				MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan )
{
	long dimension[2];
	...
	if ( energy_mcs_quick_scan == NULL )
		return;

	if ( energy_mcs_quick_scan->motor_position_array != NULL ) {
	    for ( i = 0; i < scan->num_motors; i++ ) {
		if ( energy_mcs_quick_scan->motor_position_array[i] != NULL ) {
		    mx_free( energy_mcs_quick_scan->motor_position_array[i] );

		    energy_mcs_quick_scan->motor_position_array[i] = NULL;
		}
	    }
	}
...
mx_status_type
mxs_energy_mcs_quick_scan_get_pointers( MX_SCAN *scan,
				MX_QUICK_SCAN **quick_scan,
				MX_ENERGY_MCS_QUICK_SCAN **energy_mcs_quick_scan,
				const char *calling_fname )
{
	static const char fname[] = "mxs_energy_mcs_quick_scan_get_pointers()";
	...
	if ( energy_mcs_quick_scan == (MX_ENERGY_MCS_QUICK_SCAN **) NULL ) {
		return mx_error( MXE_NULL_ARGUMENT, fname,
		"The MX_ENERGY_MCS_QUICK_SCAN pointer passed by '%s' was NULL.",
			calling_fname );
	}
	...
	*energy_mcs_quick_scan = (MX_ENERGY_MCS_QUICK_SCAN *)
					scan->record->record_type_struct;

	if ( *energy_mcs_quick_scan == (MX_ENERGY_MCS_QUICK_SCAN *) NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"The MX_ENERGY_MCS_QUICK_SCAN pointer for scan '%s' is NULL.",
			scan->record->name );
	}
...
mxs_energy_mcs_quick_scan_initialize_type( long type )
{
...
mxs_energy_mcs_quick_scan_create_record_structures( MX_RECORD *record )
{
	...
	energy_mcs_quick_scan = (MX_ENERGY_MCS_QUICK_SCAN *)
				malloc( sizeof(MX_ENERGY_MCS_QUICK_SCAN) );

	if ( energy_mcs_quick_scan == (MX_ENERGY_MCS_QUICK_SCAN *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Can't allocate memory for MX_ENERGY_MCS_QUICK_SCAN structure.");
	}
	...
	record->record_type_struct = energy_mcs_quick_scan;
	...
	for ( i = 0; i < MXS_SQ_ENERGY_MCS_MAX_MOTORS; i++ ) {
		energy_mcs_quick_scan->motor_position_array[i] = NULL;
	}

	energy_mcs_quick_scan->extension_ptr = NULL;
...
mxs_energy_mcs_quick_scan_finish_record_initialization( MX_RECORD *record )
{
	...
	status = mxs_energy_mcs_quick_scan_get_pointers( scan,
			&quick_scan, &energy_mcs_quick_scan, fname );

	if ( status.code != MXE_SUCCESS )
		return status;

	energy_mcs_quick_scan->num_mcs = 0;
	energy_mcs_quick_scan->mcs_record_array = NULL;
	energy_mcs_quick_scan->mcs_array_size = 0;

	if ( scan->num_input_devices <= 0 )
		return MX_SUCCESSFUL_RESULT;

	if ( ( scan->num_motors < 0 )
	  || ( scan->num_motors > MXS_SQ_ENERGY_MCS_MAX_MOTORS ) )
	{
		return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
		"Illegal number of motors %ld for scan '%s'.  "
		"The allowed range is (0-%d)",
			scan->num_motors, record->name,
			MXS_SQ_ENERGY_MCS_MAX_MOTORS );
	}
	...
	new_array_size = MXS_SQ_ENERGY_MCS_ARRAY_BLOCK_SIZE;

	energy_mcs_quick_scan->mcs_record_array = (MX_RECORD **)
		malloc( new_array_size * sizeof(MX_RECORD *) );

	if ( energy_mcs_quick_scan->mcs_record_array == (MX_RECORD **) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
	...
	energy_mcs_quick_scan->mcs_array_size = new_array_size;
	energy_mcs_quick_scan->num_mcs = 0;
	...
	"Input device '%s' for MCS scan '%s' is not a MCS scaler record.",
		...
		num_mcs = (long) energy_mcs_quick_scan->num_mcs;

		for ( j = 0; j < num_mcs; j++ ) {

			if (mcs_record == energy_mcs_quick_scan->mcs_record_array[j])
			{
...
"MCS record '%s' for MCS scaler '%s' is not a multichannel scaler record.",
		...
		energy_mcs_quick_scan->mcs_record_array[ num_mcs ] = mcs_record;

		(energy_mcs_quick_scan->num_mcs)++;

		/* If we have just used the last element of the array,
		 * enlarge the array.
		 */

		if ( energy_mcs_quick_scan->num_mcs >= energy_mcs_quick_scan->mcs_array_size )
		{
			new_array_size = energy_mcs_quick_scan->mcs_array_size
						+ MXS_SQ_ENERGY_MCS_ARRAY_BLOCK_SIZE;

			ptr = realloc( energy_mcs_quick_scan->mcs_record_array,
			...
			energy_mcs_quick_scan->mcs_record_array = ptr;
			energy_mcs_quick_scan->mcs_array_size = new_array_size;
		}
	}
...
MX_EXPORT mx_status_type
mxs_energy_mcs_quick_scan_delete_record( MX_RECORD *record )
{
	MX_SCAN *scan;
	MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan;
	...
	energy_mcs_quick_scan = (MX_ENERGY_MCS_QUICK_SCAN *) record->record_type_struct;

	if ( energy_mcs_quick_scan != NULL ) {
		if ( energy_mcs_quick_scan->extension_ptr != NULL ) {
			mx_free( energy_mcs_quick_scan->extension_ptr );
		}
		if ( energy_mcs_quick_scan->mcs_record_array != NULL ) {
			mx_free( energy_mcs_quick_scan->mcs_record_array );
		}
...
mxs_energy_mcs_quick_scan_set_motor_speeds( MX_SCAN *scan,
					MX_QUICK_SCAN *quick_scan,
					MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan )
{
	...
	for ( i = 0; i < scan->num_motors; i++ ) {
		...
		status = mx_motor_set_speed_between_positions(
					...
					energy_mcs_quick_scan->scan_body_time );
...
static mx_status_type
mxs_energy_mcs_quick_scan_compute_backlash_position(
			MX_RECORD *motor_record,
			double start_position,
			double end_position,
			double *backlash_position )
{
	static const char fname[] = "mxs_energy_mcs_quick_scan_compute_backlash_position()";
...
static mx_status_type
mxs_energy_mcs_quick_scan_compute_scan_parameters(
				MX_SCAN *scan,
				MX_QUICK_SCAN *quick_scan,
				MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan,
				double measurement_time_per_point )
{
	...
	status = mxs_energy_mcs_quick_scan_set_motor_speeds( scan,
						quick_scan, energy_mcs_quick_scan );
	...
	if ( longest_acceleration_time < 1.0e-9 ) {
		...
		energy_mcs_quick_scan->premove_measurement_time = 0.0;
		energy_mcs_quick_scan->postmove_measurement_time = 0.0;
		energy_mcs_quick_scan->acceleration_time = 0.0;
		energy_mcs_quick_scan->deceleration_time = 0.0;
		...
		energy_mcs_quick_scan->premove_measurement_time
				= measurement_time_per_point
					* MXS_SQ_ENERGY_MCS_NUM_PREMOVE_MEASUREMENTS;

		energy_mcs_quick_scan->postmove_measurement_time
				= energy_mcs_quick_scan->premove_measurement_time;

		quick_scan->estimated_scan_duration
			= 2.0 * energy_mcs_quick_scan->premove_measurement_time;

		/* Add in the acceleration and deceleration intervals. */

		energy_mcs_quick_scan->acceleration_time = longest_acceleration_time;
		energy_mcs_quick_scan->deceleration_time = longest_acceleration_time;
	...
	MX_DEBUG( 2,("%s: energy_mcs_quick_scan->premove_measurement_time = %g",
		fname, energy_mcs_quick_scan->premove_measurement_time));

	MX_DEBUG( 2,("%s: energy_mcs_quick_scan->acceleration_time = %g",
		fname, energy_mcs_quick_scan->acceleration_time));

	MX_DEBUG( 2,("%s: energy_mcs_quick_scan->scan_body_time = %g",
		fname, energy_mcs_quick_scan->scan_body_time));

	MX_DEBUG( 2,("%s: energy_mcs_quick_scan->deceleration_time = %g",
		fname, energy_mcs_quick_scan->deceleration_time));

	MX_DEBUG( 2,("%s: energy_mcs_quick_scan->postmove_measurement_time = %g",
		fname, energy_mcs_quick_scan->postmove_measurement_time));
	...
	for ( i = 0; i < scan->num_motors; i++ ) {
		...
		energy_mcs_quick_scan->real_start_position[i] = real_start_position;
		energy_mcs_quick_scan->real_end_position[i] = real_end_position;
		...
		MX_DEBUG( 2,("%s: motor '%s' real_start_position = %g",
			fname, motor_record->name,
			(energy_mcs_quick_scan->real_start_position)[i] ));

		MX_DEBUG( 2,("%s: motor '%s' real_end_position = %g",
			fname, motor_record->name,
			(energy_mcs_quick_scan->real_end_position)[i] ));

		/* Also compute the quick scan backlash_position. */

		status = mxs_energy_mcs_quick_scan_compute_backlash_position(
				motor_record,
				real_start_position,
				real_end_position,
				&backlash_position );
		...
		energy_mcs_quick_scan->backlash_position[i] = backlash_position;

		MX_DEBUG( 2,("%s: motor '%s' backlash_position = %g",
			fname, motor_record->name,
			(energy_mcs_quick_scan->backlash_position)[i] ));
	}
...
static MX_RECORD *
mxs_energy_mcs_quick_scan_find_encoder_readout( MX_RECORD *motor_record )
{
...
static mx_status_type
mxs_energy_mcs_quick_scan_use_dead_reckoning(
		MX_SCAN *scan,
		MX_QUICK_SCAN *quick_scan,
		MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan,
		long motor_index,
...
{
	static const char fname[] = "mxs_energy_mcs_quick_scan_use_dead_reckoning()";
	...
	pseudomotor_real_start_position
		= (energy_mcs_quick_scan->real_start_position)[motor_index];
	pseudomotor_real_end_position
		= (energy_mcs_quick_scan->real_end_position)[motor_index];
	...
	first_acceleration_measurement = MXS_SQ_ENERGY_MCS_NUM_PREMOVE_MEASUREMENTS;
	...
	acceleration_time = energy_mcs_quick_scan->acceleration_time;
...
static mx_status_type
mxs_energy_mcs_quick_scan_use_encoder_values(
		MX_RECORD *pseudomotor_record,
		MX_QUICK_SCAN *quick_scan,
		MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan,
		...
		int read_mce )
{
	static const char fname[] = "mxs_energy_mcs_quick_scan_use_encoder_values()";
...
static mx_status_type
mxs_energy_mcs_quick_scan_convert_encoder_readings( MX_QUICK_SCAN *quick_scan,
					MX_RECORD *motor_record,
					double *motor_position_array )
{
...
static mx_status_type
mxs_energy_mcs_quick_scan_compute_pseudomotor_start_from_scan_start(
		...
		MX_MCS *mcs )
{
	...
	motor_position_array = energy_mcs_quick_scan->motor_position_array;
	...
	for ( j = 0; j < scan->num_motors; j++ ) {
		...
		mce_record = mxs_energy_mcs_quick_scan_find_encoder_readout(
...
"*********************************************************************\n",
	...
	"Press OK to continue with the dead reckoning calculation.",
			...
			(void) mxs_energy_mcs_quick_scan_use_dead_reckoning(
					scan,
					quick_scan,
					energy_mcs_quick_scan,
					j,
...
"Computing positions for motor '%s' using multichannel encoder '%s'",
			...
			(void) mxs_energy_mcs_quick_scan_use_encoder_values(
					motor_record,
					quick_scan,
					energy_mcs_quick_scan,
					energy_mcs_quick_scan->real_start_position[j],
					mce_record,
					motor_position_array[j],
					TRUE );
		...
		if ( motor->motor_flags & MXF_MTR_IS_PSEUDOMOTOR ) {
			(void) mxs_energy_mcs_quick_scan_convert_encoder_readings(
					quick_scan,
					motor_record,
					motor_position_array[j] );
		}
...
static mx_status_type
mxs_energy_mcs_quick_scan_move_absolute_and_wait( MX_SCAN *scan,
					double *position_array )
{
#if DEBUG_PAUSE_REQUEST
	static const char fname[] =
		"mxs_energy_mcs_quick_scan_move_absolute_and_wait()";
#endif
...
"*** If you want accurate motor positions, you MUST use a step scan instead. ***\n",
	...
	status = mxs_energy_mcs_quick_scan_move_absolute_and_wait( scan,
	...
	energy_mcs_quick_scan->scan_body_time = measurement_time
	...
	status = mxs_energy_mcs_quick_scan_compute_scan_parameters(
			scan, quick_scan, energy_mcs_quick_scan,
			measurement_time );
	...
	if ( correct_for_quick_scan_backlash ) {
		...
		status = mxs_energy_mcs_quick_scan_move_absolute_and_wait( scan,
					energy_mcs_quick_scan->backlash_position );
	...
	status = mxs_energy_mcs_quick_scan_move_absolute_and_wait( scan,
					energy_mcs_quick_scan->real_start_position );
	...
	for ( i = 0; i < energy_mcs_quick_scan->num_mcs; i++ ) {

#if DEBUG_TIMING
		MX_HRT_START( timing_measurement );
#endif

		mcs_record = energy_mcs_quick_scan->mcs_record_array[i];
	...
	for ( i = 0; i < scan->num_motors; i++ ) {
		...
		energy_mcs_quick_scan->motor_position_array[i] = (double *)
...
static void
mxs_energy_mcs_quick_scan_check_for_motor_errors( MX_SCAN *scan )
{
#if 0
	static const char fname[] =
		"mxs_energy_mcs_quick_scan_check_for_motor_errors()";
#endif
...
{
	static const char fname[] = "mxs_energy_mcs_quick_scan_execute_scan_body()";
	...
	mx_status = mxs_energy_mcs_quick_scan_get_pointers( scan,
			&quick_scan, &energy_mcs_quick_scan, fname );
	...
	if ( energy_mcs_quick_scan->num_mcs == 1 ) {
	...
	for ( i = 0; i < max_attempts; i++ ) {
		...
		for ( j = 0; j < energy_mcs_quick_scan->num_mcs; j++ ) {
			mcs_record = energy_mcs_quick_scan->mcs_record_array[j];
	...
	if ( i >= max_attempts ) {
		...
		"quick scan '%s' to start counting.",
			...
			energy_mcs_quick_scan->mcs_record_array[j]->name,
			scan->record->name );
	}
#endif
	 ...
	 *
	 * Note: We do not use mxs_energy_mcs_quick_scan_move_absolute_and_wait()
	...
	mx_status = mx_motor_array_move_absolute(
			scan->num_motors,
			scan->motor_record_array,
			energy_mcs_quick_scan->real_end_position,
	...
	do {
		if ( mx_user_requested_interrupt() ) {
			...
			for ( i = 0; i < energy_mcs_quick_scan->num_mcs; i++ ) {
				(void) mx_mcs_stop(
					energy_mcs_quick_scan->mcs_record_array[i] );
			}
	...
	(void) mxs_energy_mcs_quick_scan_check_for_motor_errors( scan );
	...
	for ( i = 0; i < energy_mcs_quick_scan->num_mcs; i++ ) {
		(void) mx_mcs_stop( energy_mcs_quick_scan->mcs_record_array[i] );
	}
...
mxs_energy_mcs_quick_scan_cleanup_after_scan_end( MX_SCAN *scan )
{
	...
	MX_ENERGY_MCS_QUICK_SCAN *energy_mcs_quick_scan;
	long **data_array;
	double motor_datafile_positions[ MXS_SQ_ENERGY_MCS_MAX_MOTORS ];
	double motor_plot_positions[ MXS_SQ_ENERGY_MCS_MAX_MOTORS ];
	double measurement_time;
	long *data_values;
	...
	mx_status = mxs_energy_mcs_quick_scan_compute_motor_positions(
			scan, quick_scan, energy_mcs_quick_scan, NULL );
	...
	for ( i = 0; i < quick_scan->actual_num_measurements; i++ ) {
		...
		if ( scan->datafile.num_x_motors > 0 ) {
			...
			for ( j = 0; j < num_datafile_motors; j++ ) {
				motor_datafile_positions[j] = 
				    energy_mcs_quick_scan->motor_position_array[j][i];
			}
		}
