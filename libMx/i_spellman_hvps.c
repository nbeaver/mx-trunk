/*
 * Name:    i_spellman_hvps.c
 *
 * Purpose: MX interface driver for Spellman high voltage power supplies
...
#define MXI_SPELLMAN_HVPS_DEBUG	TRUE
...
MX_RECORD_FUNCTION_LIST mxi_spellman_hvps_record_function_list = {
	NULL,
	mxi_spellman_hvps_create_record_structures,
	NULL,
	NULL,
	NULL,
	NULL,
	NULL,
	mxi_spellman_hvps_open,
	NULL,
	NULL,
	mxi_spellman_hvps_resynchronize
};

MX_RECORD_FIELD_DEFAULTS mxi_spellman_hvps_record_field_defaults[] = {
	MX_RECORD_STANDARD_FIELDS,
	MXI_SPELLMAN_HVPS_STANDARD_FIELDS
};

long mxi_spellman_hvps_num_record_fields
		= sizeof( mxi_spellman_hvps_record_field_defaults )
			/ sizeof( mxi_spellman_hvps_record_field_defaults[0] );

MX_RECORD_FIELD_DEFAULTS *mxi_spellman_hvps_rfield_def_ptr
			= &mxi_spellman_hvps_record_field_defaults[0];

MX_EXPORT mx_status_type
mxi_spellman_hvps_create_record_structures( MX_RECORD *record )
{
	...
	spellman_hvps = (MX_SPELLMAN_HVPS *) malloc( sizeof(MX_SPELLMAN_HVPS) );

	if ( spellman_hvps == (MX_SPELLMAN_HVPS *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Can't allocate memory for MX_SPELLMAN_HVPS structure." );
	}

	/* Now set up the necessary pointers. */

	record->record_class_struct = NULL;
	record->record_type_struct = spellman_hvps;

	record->record_function_list = &mxi_spellman_hvps_record_function_list;
	record->superclass_specific_function_list = NULL;
	record->class_specific_function_list = NULL;

	spellman_hvps->record = record;

	return MX_SUCCESSFUL_RESULT;
}

MX_EXPORT mx_status_type
mxi_spellman_hvps_open( MX_RECORD *record )
{
	static const char fname[] = "mxi_spellman_hvps_open()";
	...
	spellman_hvps = (MX_SPELLMAN_HVPS *) record->record_type_struct;

	if ( spellman_hvps == (MX_SPELLMAN_HVPS *) NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"The MX_SPELLMAN_HVPS pointer for record '%s' is NULL.",
			record->name);
	}

	/* Compute the query command interval in clock ticks. */

	if ( spellman_hvps->query_interval > 0 ) {
		spellman_hvps->ticks_per_query =
	    mx_convert_seconds_to_clock_ticks( spellman_hvps->query_interval );

		spellman_hvps->next_query_tick = mx_current_clock_tick();
	}

	/* Clear out any existing trash from the RS-232 line. */

	mx_status = mx_rs232_discard_unread_input( spellman_hvps->rs232_record,
						MXI_SPELLMAN_HVPS_DEBUG );
	...
	mx_status = mxi_spellman_hvps_command( spellman_hvps, "V",
	...
	if ( num_items != 1 ) {
		...
		"Spellman HVPS '%s' was not recognizable.",
			response, record->name );
	}

#if MXI_SPELLMAN_HVPS_DEBUG
	MX_DEBUG(-2,("%s: Spellman HVPS '%s' software version = %lu",
	...
	spellman_hvps->analog_control[MXF_SPELLMAN_HVPS_VOLTAGE_CONTROL] =
	  4 * spellman_hvps->analog_monitor[MXF_SPELLMAN_HVPS_VOLTAGE_MONITOR];

	spellman_hvps->analog_control[MXF_SPELLMAN_HVPS_CURRENT_CONTROL] =
	  4 * spellman_hvps->analog_monitor[MXF_SPELLMAN_HVPS_CURRENT_MONITOR];

	/* Copy in the default power and filament current limits. */

	spellman_hvps->analog_control[MXF_SPELLMAN_HVPS_POWER_LIMIT]
		= spellman_hvps->default_power_limit;

	spellman_hvps->analog_control[MXF_SPELLMAN_HVPS_FILAMENT_CURRENT_LIMIT]
		= spellman_hvps->default_filament_current_limit;
...
MX_EXPORT mx_status_type
mxi_spellman_hvps_resynchronize( MX_RECORD *record )
{
	static const char fname[] = "mxi_spellman_hvps_resynchronize()";
	...
	if ( spellman_hvps == (MX_SPELLMAN_HVPS *) NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"MX_SPELLMAN_HVPS pointer for record '%s' is NULL.",
			record->name);
	}

	mx_status = mx_resynchronize_record( spellman_hvps->rs232_record );

	return mx_status;
}

MX_EXPORT mx_status_type
mxi_spellman_hvps_command( MX_SPELLMAN_HVPS *spellman_hvps,
			char *command,
			char *response,
			size_t response_buffer_length,
			int debug_flag )
{
	static const char fname[] = "mxi_spellman_hvps_command()";
	...
	if ( spellman_hvps == (MX_SPELLMAN_HVPS *) NULL ) {
		return mx_error( MXE_NULL_ARGUMENT, fname,
		"The MX_SPELLMAN_HVPS pointer passed was NULL." );
	}

	if ( debug_flag ) {
		MX_DEBUG(-2,("%s: sending '%s' to '%s'",
			fname, command, spellman_hvps->record->name ));
	}
	...
	mx_status = mx_rs232_putline( spellman_hvps->rs232_record,
	...
	mx_status = mx_rs232_getline( spellman_hvps->rs232_record,
	...
	if ( actual_checksum != expected_checksum ) {
		...
		"Spellman HVPS '%s' had a checksum %#lX that did not match "
		"the expected checksum of %#lX.  Continuing anyway.",
			response, command, spellman_hvps->record->name,
	...
	*checksum_ptr = '\0';
	...
	if ( debug_flag ) {
		MX_DEBUG(-2,("%s: received '%s' from '%s'",
			fname, response, spellman_hvps->record->name ));
	}
...
MX_EXPORT mx_status_type
mxi_spellman_hvps_query_command( MX_SPELLMAN_HVPS *spellman_hvps )
{
	static const char fname[] = "mxi_spellman_hvps_query_command()";
	...
	/* If the 'query_interval' field for the 'spellman_hvps' record
	 ...
	 * 'digital_monitor' arrays of the MX_SPELLMAN_HVPS structure.
	 */
	...
	if ( spellman_hvps->query_interval > 0 ) {
		...
		spellman_hvps->next_query_tick = mx_add_clock_ticks(
			current_tick, spellman_hvps->ticks_per_query );
	}

	/* Send the query command. */

	mx_status = mxi_spellman_hvps_command( spellman_hvps, "Q",
					response, sizeof(response),
					MXI_SPELLMAN_HVPS_DEBUG );
	...
	if ( response[0] != 'R' ) {
		...
		"was not the letter 'R' for Spellman HVPS '%s'.",
			response, spellman_hvps->record->name );
	}

	spellman_hvps->analog_monitor[MXF_SPELLMAN_HVPS_VOLTAGE_MONITOR] =
	...
	spellman_hvps->analog_monitor[MXF_SPELLMAN_HVPS_CURRENT_MONITOR] =
	...
	spellman_hvps->analog_monitor[
				MXF_SPELLMAN_HVPS_FILAMENT_CURRENT_MONITOR] =
	...
	if ( monitor_byte & 0x8 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_INTERLOCK_STATUS]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_INTERLOCK_STATUS]
	    							= FALSE;
	...
	if ( monitor_byte & 0x4 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_XRAY_ON_INDICATOR]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_XRAY_ON_INDICATOR]
	    							= FALSE;
	...
	if ( monitor_byte & 0x2 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_POWER_SUPPLY_FAULT]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_POWER_SUPPLY_FAULT]
	    							= FALSE;
	...
	if ( monitor_byte & 0x1 ) {
	    spellman_hvps->digital_monitor[
	    	MXF_SPELLMAN_HVPS_FILAMENT_CURRENT_LIMIT_FAULT] = TRUE;
	} else {
	    spellman_hvps->digital_monitor[
	    	MXF_SPELLMAN_HVPS_FILAMENT_CURRENT_LIMIT_FAULT] = FALSE;
	}

	monitor_byte = mx_hex_char_to_unsigned_long( response[11] );

	if ( monitor_byte & 0x8 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERVOLTAGE_FAULT]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERVOLTAGE_FAULT]
	    							= FALSE;
	...
	if ( monitor_byte & 0x4 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERPOWER_FAULT]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERPOWER_FAULT]
	    							= FALSE;
	...
	if ( monitor_byte & 0x2 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERCURRENT_FAULT]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_OVERCURRENT_FAULT]
	    							= FALSE;
	...
	if ( monitor_byte & 0x1 ) {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_KV_MINIMUM_FAULT]
							    	= TRUE;
	} else {
	    spellman_hvps->digital_monitor[MXF_SPELLMAN_HVPS_KV_MINIMUM_FAULT]
	    							= FALSE;
	...
	if ( monitor_byte & 0x1 ) {
	    spellman_hvps->digital_monitor[
	    	MXF_SPELLMAN_HVPS_CONTROL_MODE_INDICATOR] = TRUE;
	} else {
	    spellman_hvps->digital_monitor[
	    	MXF_SPELLMAN_HVPS_CONTROL_MODE_INDICATOR] = FALSE;
	}
...
#if 0
	{
		...
		for ( i = 0; i < num_digital_monitors; i++ ) {
			MX_DEBUG(-2,("%s: digital_monitor[%d] = %d",
				fname, i, spellman_hvps->digital_monitor[i]));
		}
	}
#endif
...
MX_EXPORT mx_status_type
mxi_spellman_hvps_set_command( MX_SPELLMAN_HVPS *spellman_hvps )
{
	static const char fname[] = "mxi_spellman_hvps_set_command()";
	...
	digital_monitor = spellman_hvps->digital_monitor;

	fault = digital_monitor[MXF_SPELLMAN_HVPS_KV_MINIMUM_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_OVERCURRENT_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_OVERPOWER_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_OVERVOLTAGE_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_FILAMENT_CURRENT_LIMIT_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_POWER_SUPPLY_FAULT]
	    | digital_monitor[MXF_SPELLMAN_HVPS_INTERLOCK_STATUS];

	/* Construct the command to be sent to the power supply.
	 *
	 * The leading ^A character and trailing checksum and ^M will be
	 * added by the function mxi_spellman_hvps_command().
	 */
	...
	digital_control = spellman_hvps->digital_control;
	...
	mx_status = mxi_spellman_hvps_command( spellman_hvps, command,
						response, sizeof(response),
						MXI_SPELLMAN_HVPS_DEBUG );
	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;
	...
	"for Spellman HVPS '%s'.", response, command,
		spellman_hvps->record->name );
}
