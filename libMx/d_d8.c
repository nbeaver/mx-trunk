...
mxd_d8_motor_open( MX_RECORD *record )
{
	...
	sprintf( command, "AV%d", (int) d8_motor->drive_number );
	...
	if ( strcmp( response, "AV0.--" ) == 0 ) {
		...
		MX_DEBUG(-2,("%s: Axis %d is not initialized yet.",
			fname, (int) d8_motor->drive_number));

		sprintf( command, "AV%d,0.00", (int) d8_motor->drive_number );
...
mxd_d8_motor_motor_is_busy( MX_MOTOR *motor )
{
	...
	sprintf( command, "ST%d", 5 + (int) d8_motor->drive_number );
...
mxd_d8_motor_move_absolute( MX_MOTOR *motor )
{
	...
	sprintf( command, "GO%d,%g,%g", (int) d8_motor->drive_number,
...
mxd_d8_motor_set_position( MX_MOTOR *motor )
{
	...
	sprintf( command, "AV%d,%g", (int) d8_motor->drive_number,
	...
	sprintf( command, "ZI%d,0", (int) d8_motor->drive_number );
	...
	sprintf( command, "IN%d", (int) d8_motor->drive_number );
...
mxd_d8_motor_soft_abort( MX_MOTOR *motor )
{
	...
	sprintf( command, "HT%d", (int) d8_motor->drive_number );
...
mxd_d8_motor_find_home_position( MX_MOTOR *motor )
{
	...
	if ( motor->home_search >= 0 ) {
		sprintf( command, "FR%d,U", (int) d8_motor->drive_number );
	} else {
		sprintf( command, "FR%d,D", (int) d8_motor->drive_number );
	}
