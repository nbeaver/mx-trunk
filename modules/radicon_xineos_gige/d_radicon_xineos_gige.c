...
#define MXD_RADICON_XINEOS_GIGE_DEBUG_RS232				TRUE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_SRO_SI			FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_SRO_SI_SUMMARY		TRUE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_DELAY			FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS		FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS_WHEN_BUSY	FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS_WHEN_CHANGED	TRUE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_READOUT_TIMING			FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_SAVING_RAW_FILES_SETUP		FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_SAVING_RAW_FILES		FALSE

#define MXD_RADICON_XINEOS_GIGE_DEBUG_RAW_FRAME_IS_SAVED		FALSE
...
MX_RECORD_FUNCTION_LIST mxd_radicon_xineos_gige_record_function_list = {
	...
	NULL,
	NULL,
	mxd_radicon_xineos_gige_resynchronize,
	mxd_radicon_xineos_gige_special_processing_setup
};
...
static mx_status_type mxd_radicon_xineos_gige_process_function( void *record_ptr,
...
mxd_radicon_xineos_gige_open( MX_RECORD *record )
{
	...
	radicon_xineos_gige->poll_pulser_status = FALSE;

	if ( strlen( radicon_xineos_gige->pulser_record_name ) <= 0 ) {
		radicon_xineos_gige->pulser_record = NULL;

	} else {
		radicon_xineos_gige->pulser_record = mx_get_record( ad->record,
					radicon_xineos_gige->pulser_record_name );

		if ( radicon_xineos_gige->pulser_record == (MX_RECORD *) NULL ) {
		...
		if ( radicon_xineos_gige->pulser_record->mx_class
			!= MXC_PULSE_GENERATOR )
		{
			...
			"pulse generator.",
				radicon_xineos_gige->pulser_record->name,
				ad->record->name );
		...
		radicon_xineos_gige->poll_pulser_status = TRUE;
	}
	...
	MX_DEBUG(-2,("%s: radicon_xineos_gige->poll_pulser_status = %d",
		fname, (int) radicon_xineos_gige->poll_pulser_status ));
	...
	serial_port_record = radicon_xineos_gige->serial_port_record;
	...
	mx_status = mx_rs232_discard_unwritten_output( serial_port_record,
					MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
	...
	radicon_xineos_gige->serial_delay_ticks =
		mx_convert_seconds_to_clock_ticks( serial_delay_in_seconds );

	radicon_xineos_gige->next_serial_command_tick = mx_current_clock_tick();

	/* Request camera parameters from the detector. */

	mx_status = mxd_radicon_xineos_gige_command( radicon_xineos_gige, "gcp",
				NULL, 0, MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
	...
	radicon_xineos_gige->detector_model = 0;
	radicon_xineos_gige->serial_number[0] = '\0';
	radicon_xineos_gige->firmware_version = 0;
	response_seen = FALSE;
	...
	while (1) {
		mx_status = mx_rs232_num_input_bytes_available(
					radicon_xineos_gige->serial_port_record,
					&num_bytes_available );
		...
		mx_status = mx_rs232_getline(radicon_xineos_gige->serial_port_record,
		...
		if ( strncmp( response, "Detector Model", 14 ) == 0 ) {
			if ( strcmp( string_value_ptr, "XINEOS_GIGE" ) == 0 ) {

				radicon_xineos_gige->detector_model
					= MXT_RADICON_XINEOS_GIGE;
			} else {
		...
		if ( strncmp( response, "Detector S/N", 12 ) == 0 ) {
			strlcpy( radicon_xineos_gige->serial_number,
				string_value_ptr,
				MXU_RADICON_XINEOS_GIGE_SERIAL_NUMBER_LENGTH );
		} else
		...
		if ( strncmp(response, "Detector FW Build Version", 25) == 0 ) {
			radicon_xineos_gige->firmware_version =
		...
		if ( radicon_xineos_gige->detector_model == 0 ) {
	...
	if ( num_bytes_available == 1 ) {
		mx_status = mx_rs232_getchar(
				radicon_xineos_gige->serial_port_record,
				&c, MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
	...
	if ( radicon_xineos_gige->firmware_version >= 105 ) {
		radicon_xineos_gige->have_get_commands = TRUE;
	} else {
		radicon_xineos_gige->have_get_commands = FALSE;
	}

	/* Set the detector gain to 1. */

	if ( radicon_xineos_gige->firmware_version >= 105 ) {
		mx_status = mxd_radicon_xineos_gige_command( radicon_xineos_gige,
				"i2caw 72 1 0", NULL, 0, 
				MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
	...
	radicon_xineos_gige->sro_mode = 4;

	mx_status = mxd_radicon_xineos_gige_set_sro( ad );
	...
	flags = radicon_xineos_gige->radicon_xineos_gige_flags;

	if ( flags & MXF_RADICON_XINEOS_GIGE_DO_NOT_FLIP_IMAGE ) {
		radicon_xineos_gige->flip_image = FALSE;
	} else {
		radicon_xineos_gige->flip_image = TRUE;
	}
	...
	if ( radicon_xineos_gige->use_video_frames ) {
	...
	if ( radicon_xineos_gige->firmware_version < 105 ) {
		radicon_xineos_gige->linetime = 946;
	} else {
		/* 677 gives us a 30 Hz frame rate. */

		radicon_xineos_gige->linetime = 677;

		snprintf( command, sizeof(command),
			"linetime %lu", radicon_xineos_gige->linetime );
	...
	radicon_xineos_gige->clock_frequency = 30.0e6;

	radicon_xineos_gige->readout_time = ( video_framesize[1] / 2 )
		* radicon_xineos_gige->linetime / radicon_xineos_gige->clock_frequency;

	/*---*/

	radicon_xineos_gige->si1_si2_ratio = 0.1;

	radicon_xineos_gige->use_different_si2_value = FALSE;

	/* Seed the registers with the minimum legal values. */

	radicon_xineos_gige->si1_register = 4;
	radicon_xineos_gige->si2_register = 4;

	/* If possible, fetch the current settings from the real registers. */

	mx_status = mxd_radicon_xineos_gige_get_si1( ad );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	mx_status = mxd_radicon_xineos_gige_get_si2( ad );
	...
	radicon_xineos_gige->bypass_arm = FALSE;

	radicon_xineos_gige->old_total_num_frames = -1;
	radicon_xineos_gige->old_status = 0xffffffff;
	...
	mx_status = mx_video_input_get_num_capture_buffers(
					video_input_record,
					&(radicon_xineos_gige->num_capture_buffers));
	...
	radicon_xineos_gige->buffer_info_array = (MX_RADICON_XINEOS_GIGE_BUFFER_INFO *)
		calloc( radicon_xineos_gige->num_capture_buffers,
			sizeof(MX_RADICON_XINEOS_GIGE_BUFFER_INFO) );

	if ( radicon_xineos_gige->buffer_info_array ==
		(MX_RADICON_XINEOS_GIGE_BUFFER_INFO *) NULL )
	{
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Ran out of memory trying to allocate a %lu element array "
		"of MX_RADICON_XINEOS_GIGE_BUFFER_INFO structures.",
			radicon_xineos_gige->num_capture_buffers );
	}
	...
	mx_status = mx_image_alloc( &(radicon_xineos_gige->video_frame),
					vinput_framesize[0],
					vinput_framesize[1],
					ad->image_format,
					ad->byte_order,
	...
	MXIF_ROW_BINSIZE(radicon_xineos_gige->video_frame) = 1;
	MXIF_COLUMN_BINSIZE(radicon_xineos_gige->video_frame) = 1;
	MXIF_BITS_PER_PIXEL(radicon_xineos_gige->video_frame) = ad->bits_per_pixel;
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_resynchronize( MX_RECORD *record )
{
	static const char fname[] = "mxd_radicon_xineos_gige_resynchronize()";
	...
	if ( radicon_xineos_gige->pulser_record != NULL ) {
		mx_status = mx_pulse_generator_stop(
				radicon_xineos_gige->pulser_record );
	}

	/* Abort image acquisition. */

	mx_status = mxd_radicon_xineos_gige_abort( ad );
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_special_processing_setup( MX_RECORD *record )
{
	static const char fname[] =
		"mxd_radicon_xineos_gige_special_processing_setup()";
	...
	for ( i = 0; i < record->num_record_fields; i++ ) {
		...
		case MXLV_RADICON_XINEOS_GIGE_SRO:
		case MXLV_RADICON_XINEOS_GIGE_SI1:
		case MXLV_RADICON_XINEOS_GIGE_SI2:
			record_field->process_function
					= mxd_radicon_xineos_gige_process_function;
			break;
		default:
			break;
		}
...
static mx_status_type
mxd_radicon_xineos_gige_generate_throwaway_frame( MX_AREA_DETECTOR *ad,
					MX_RADICON_XINEOS_GIGE *radicon_xineos_gige )
{
	static const char fname[] =
			"mxd_radicon_xineos_gige_generate_throwaway_frame()";
	...
	radicon_xineos_gige->si1_register = 500;

	mx_status = mxd_radicon_xineos_gige_set_si1( ad ); 

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	radicon_xineos_gige->si2_register = 500;

	mx_status = mxd_radicon_xineos_gige_set_si2( ad ); 
	...
	mx_status = mx_pulse_generator_set_mode( radicon_xineos_gige->pulser_record,
							MXF_PGN_PULSE );
	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	mx_status = mx_pulse_generator_set_pulse_period(
					radicon_xineos_gige->pulser_record, 0.1 );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	mx_status = mx_pulse_generator_set_pulse_width(
					radicon_xineos_gige->pulser_record, 0.05 );
	...
	mx_status = mx_pulse_generator_start( radicon_xineos_gige->pulser_record );
	...
	mx_status = mx_pulse_generator_stop( radicon_xineos_gige->pulser_record );
...
mxd_radicon_xineos_gige_arm( MX_AREA_DETECTOR *ad )
{
	...
	memset( radicon_xineos_gige->buffer_info_array, 0,
		radicon_xineos_gige->num_capture_buffers
			  * sizeof(MX_RADICON_XINEOS_GIGE_BUFFER_INFO) );
	...
	if ( radicon_xineos_gige->bypass_arm ) {
	...
	switch( radicon_xineos_gige->detector_model ) {
	case MXT_RADICON_XINEOS_GIGE:

		old_sro_mode = radicon_xineos_gige->sro_mode;
		...
		use_different_si2_value =
			radicon_xineos_gige->use_different_si2_value;
		...
		case MXT_SQ_ONE_SHOT:
			...
			radicon_xineos_gige->sro_mode = 3;
			break;
		...
		case MXT_SQ_STROBE:
			...
			if ( use_different_si2_value ) {
				radicon_xineos_gige->sro_mode = 0;
			} else {
				radicon_xineos_gige->sro_mode = 1;
			}
			break;
		case MXT_SQ_DURATION:
			...
			radicon_xineos_gige->sro_mode = 2;
			break;
		case MXT_SQ_GATED:
			set_exposure_times = TRUE;
		...
		if ( radicon_xineos_gige->sro_mode != old_sro_mode ) {
		...
		long_exposure_time_as_double = 
			(exposure_time - radicon_xineos_gige->readout_time)
				 * radicon_xineos_gige->clock_frequency;

		if ( long_exposure_time_as_double <
			radicon_xineos_gige->minimum_exposure_ticks )
		{
			long_exposure_time_as_double =
				radicon_xineos_gige->minimum_exposure_ticks;
		}
		...
		if ( use_different_si2_value ) {
			short_exposure_time_as_double =
				radicon_xineos_gige->si1_si2_ratio
					* long_exposure_time_as_double;
		...
		radicon_xineos_gige->si1_register = si1_register;
		...
		radicon_xineos_gige->si2_register = si2_register;
	...
	case MXT_RADICON_XINEOS:
		...
		mx_status = mxd_radicon_xineos_gige_command( radicon_xineos_gige, command,
				NULL, 0, MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_abort( MX_AREA_DETECTOR *ad )
{
	static const char fname[] = "mxd_radicon_xineos_gige_abort()";
	...
	MX_DEBUG(-2,("%s invoked for area detector '%s'.",
		fname, ad->record->name ));
#endif
	mx_status = mx_video_input_abort( radicon_xineos_gige->video_input_record );
...
mxd_radicon_xineos_gige_get_extended_status( MX_AREA_DETECTOR *ad )
{
	...
	if ( radicon_xineos_gige->poll_pulser_status ) {
...
#if MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS
...
#elif MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS_WHEN_BUSY
...
#elif MXD_RADICON_XINEOS_GIGE_DEBUG_EXTENDED_STATUS_WHEN_CHANGED

	if ( (ad->total_num_frames != radicon_xineos_gige->old_total_num_frames)
	  || (ad->status           != radicon_xineos_gige->old_status) )
	{
		MX_DEBUG(-2,("%s: last_frame_number = %ld, "
			"total_num_frames = %ld, status = %#lx",
			fname, ad->last_frame_number,
	...
	radicon_xineos_gige->old_total_num_frames = ad->total_num_frames;
	radicon_xineos_gige->old_status           = ad->status;

	return MX_SUCCESSFUL_RESULT;
}

static mx_status_type
mxp_radicon_xineos_gige_save_raw_image( MX_AREA_DETECTOR *ad,
				MX_RADICON_XINEOS_GIGE *radicon_xineos_gige )
{
	static const char fname[] = "mxp_radicon_xineos_gige_save_raw_image()";

	MX_RADICON_XINEOS_GIGE_BUFFER_INFO *buffer_info;
	long frame_number;
...
#if MXD_RADICON_XINEOS_GIGE_DEBUG_SAVING_RAW_FILES_SETUP
	...
	buffer_info = &(radicon_xineos_gige->buffer_info_array[ frame_number ]);

#if MXD_RADICON_XINEOS_GIGE_DEBUG_RAW_FRAME_IS_SAVED
	...
	snprintf( raw_image_pathname, sizeof(raw_image_pathname),
		"%s/%s",
		radicon_xineos_gige->raw_file_directory,
		filename_temp );

	strlcpy( radicon_xineos_gige->raw_file_name, filename_temp,
		sizeof(radicon_xineos_gige->raw_file_name) );
...
#if MXD_RADICON_XINEOS_GIGE_DEBUG_SAVING_RAW_FILES
...
mxd_radicon_xineos_gige_readout_frame( MX_AREA_DETECTOR *ad )
{
	...
	if ( radicon_xineos_gige->use_video_frames ) {
		...
		mx_status = mx_video_input_get_frame(
			radicon_xineos_gige->video_input_record,
			ad->readout_frame, &(radicon_xineos_gige->video_frame) );
...
#endif
		video_row_framesize =
			MXIF_ROW_FRAMESIZE(radicon_xineos_gige->video_frame);

		video_column_framesize =
			MXIF_COLUMN_FRAMESIZE(radicon_xineos_gige->video_frame);
		...
		video_frame_buffer = radicon_xineos_gige->video_frame->image_data;
		...
		if ( radicon_xineos_gige->flip_image ) {
		...
		MXIF_EXPOSURE_TIME_SEC( ad->image_frame )
			= MXIF_EXPOSURE_TIME_SEC( radicon_xineos_gige->video_frame );

		MXIF_EXPOSURE_TIME_NSEC( ad->image_frame )
			= MXIF_EXPOSURE_TIME_NSEC( radicon_xineos_gige->video_frame);

		MXIF_TIMESTAMP_SEC( ad->image_frame )
			= MXIF_TIMESTAMP_SEC( radicon_xineos_gige->video_frame );

		MXIF_TIMESTAMP_NSEC( ad->image_frame )
			= MXIF_TIMESTAMP_NSEC( radicon_xineos_gige->video_frame );
	...
	if ( flags & MXF_RADICON_XINEOS_GIGE_SAVE_RAW_IMAGES ) {

		mx_status = mxp_radicon_xineos_gige_save_raw_image( ad,
							radicon_xineos_gige );
	}
	...
	if ( radicon_xineos_gige->use_video_frames == FALSE ) {
...
mxd_radicon_xineos_gige_set_parameter( MX_AREA_DETECTOR *ad )
{
	...
	case MXLV_AD_BINSIZE:
		...
		case MXT_RADICON_XINEOS:
			...
			if ( sro_mode != radicon_xineos_gige->sro_mode ) {
				if ( ad->binsize[0] == 1 ) {
				...
				mx_status = mxd_radicon_xineos_gige_command(
						radicon_xineos_gige,
						command, NULL, 0,
					MXD_RADICON_XINEOS_GIGE_DEBUG_RS232 );
			}
	...
	case MXLV_AD_TRIGGER_MODE:
		 ...
		 * at 'arm' time.  Look in mxd_radicon_xineos_gige_arm() for it.
		 */
		break;
...
{
	static const char fname[] = "mxd_radicon_xineos_gige_measure_correction()";
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_get_sro( MX_AREA_DETECTOR *ad )
{
	static const char fname[] = "mxd_radicon_xineos_gige_get_sro()";
	...
	if ( radicon_xineos_gige->have_get_commands == FALSE ) {
		 ...
		 * radicon_xineos_gige->sro_mode.
		 */

		return MX_SUCCESSFUL_RESULT;
	}

	mx_status = mxd_radicon_xineos_gige_command( radicon_xineos_gige, "getsro",
					response, sizeof(response),
					MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_SRO_SI );
	...
	radicon_xineos_gige->sro_mode = mx_string_to_unsigned_long( response_ptr );

#if MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_SRO_SI_SUMMARY
	MX_DEBUG(-2,("%s: SRO value read = %lu",
		fname, radicon_xineos_gige->sro_mode));
#endif
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_set_sro( MX_AREA_DETECTOR *ad )
{
	static const char fname[] = "mxd_radicon_xineos_gige_set_sro()";
	...
	MX_DEBUG(-2,("%s invoked for area detector '%s'.",
		fname, ad->record->name ));
#endif
	new_sro_mode = radicon_xineos_gige->sro_mode;
	...
	if ( new_sro_mode > 4 ) {
		...
		mx_status = mxd_radicon_xineos_gige_get_sro( ad );
	...
	for ( i = 0; i < max_attempts; i++ ) {
		...
		if ( radicon_xineos_gige->sro_mode == new_sro_mode ) {
...
static mx_status_type
mxd_radicon_xineos_gige_get_si_register( MX_AREA_DETECTOR *ad, long si_type )
{
	static const char fname[] = "mxd_radicon_xineos_gige_get_si_register()";
	...
	if ( serial_port_record == (MX_RECORD *) NULL ) {
		...
		"The serial_port_record pointer for record '%s' is NULL.",
			radicon_xineos_gige->record->name );
	}
	...
	if ( radicon_xineos_gige->have_get_commands == FALSE ) {
		 ...
		 * return the value currently in the variable
		 * radicon_xineos_gige->si1_register or
		 * radicon_xineos_gige->si2_register.
		 */
	...
	for ( i = 0; i < 3; i++ ) {
		...
		mx_status = mxd_radicon_xineos_gige_command( radicon_xineos_gige, NULL,
					response, sizeof(response),
					MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_SRO_SI );
	...
	switch( si_type ) {
	case MXLV_RADICON_XINEOS_GIGE_SI1:
		radicon_xineos_gige->si1_register = new_si_value;
		break;
	case MXLV_RADICON_XINEOS_GIGE_SI2:
		radicon_xineos_gige->si2_register = new_si_value;
		break;
	}
...
static mx_status_type
mxd_radicon_xineos_gige_set_si_register( MX_AREA_DETECTOR *ad, long si_type )
{
	static const char fname[] = "mxd_radicon_xineos_gige_set_si_register()";
	...
	switch( si_type ) {
	case MXLV_RADICON_XINEOS_GIGE_SI1:
		new_si_value = radicon_xineos_gige->si1_register;
		break;
	case MXLV_RADICON_XINEOS_GIGE_SI2:
		new_si_value = radicon_xineos_gige->si2_register;
		break;
	default:
		return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
		"Invoked with illegal si_type value %ld", si_type );
	...
	for ( i = 0; i < max_attempts; i++ ) {
		...
		mx_status = mxd_radicon_xineos_gige_get_si_register( ad, si_type );
		...
		switch( si_type ) {
		case MXLV_RADICON_XINEOS_GIGE_SI1:
			if ( radicon_xineos_gige->si1_register == new_si_value ) {
				return MX_SUCCESSFUL_RESULT;
			}
			break;
		case MXLV_RADICON_XINEOS_GIGE_SI2:
			if ( radicon_xineos_gige->si2_register == new_si_value ) {
				return MX_SUCCESSFUL_RESULT;
			}
			break;
		}
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_get_si1( MX_AREA_DETECTOR *ad )
{
	mx_status_type mx_status;

	mx_status = mxd_radicon_xineos_gige_get_si_register( ad,
						MXLV_RADICON_XINEOS_GIGE_SI1 );

	return mx_status;
}

MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_set_si1( MX_AREA_DETECTOR *ad )
{
	mx_status_type mx_status;

	mx_status = mxd_radicon_xineos_gige_set_si_register( ad,
						MXLV_RADICON_XINEOS_GIGE_SI1 );
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_get_si2( MX_AREA_DETECTOR *ad )
{
	mx_status_type mx_status;

	mx_status = mxd_radicon_xineos_gige_get_si_register( ad,
						MXLV_RADICON_XINEOS_GIGE_SI2 );

	return mx_status;
}

MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_set_si2( MX_AREA_DETECTOR *ad )
{
	mx_status_type mx_status;
...
static mx_status_type
mxd_radicon_xineos_gige_process_function( void *record_ptr,
				void *record_field_ptr,
				int operation )
{
	static const char fname[] = "mxd_radicon_xineos_gige_process_function()";
	...
	ad = (MX_AREA_DETECTOR *) record->record_class_struct;
	radicon_xineos_gige = (MX_RADICON_XINEOS_GIGE *) record->record_type_struct;
...
MX_EXPORT mx_status_type
mxd_radicon_xineos_gige_command( MX_RADICON_XINEOS_GIGE *radicon_xineos_gige, char *command,
			char *response, size_t response_buffer_length,
			mx_bool_type debug_flag )
{
	static const char fname[] = "mxd_radicon_xineos_gige_command()";
	...
	if ( radicon_xineos_gige == (MX_RADICON_XINEOS_GIGE *) NULL ) {
		return mx_error( MXE_NULL_ARGUMENT, fname,
		"The MX_RADICON_XINEOS_GIGE pointer passed was NULL." );
	}
...
#if MXD_RADICON_XINEOS_GIGE_DEBUG_RS232_DELAY
		MX_DEBUG(-2,("%s: next_serial_command_tick = (%lu,%lu)", fname,
	  (unsigned long) radicon_xineos_gige->next_serial_command_tick.high_order,
	  (unsigned long) radicon_xineos_gige->next_serial_command_tick.low_order));
#endif
		...
		while (1) {
			...
			comparison = mx_compare_clock_ticks( current_tick,
				radicon_xineos_gige->next_serial_command_tick );
	  ...
	  (unsigned long) radicon_xineos_gige->next_serial_command_tick.low_order));
		...
		if ( debug_flag ) {
			MX_DEBUG(-2,("%s: sending '%s' to '%s'",
				fname, command, radicon_xineos_gige->record->name ));
		}
	...
	if ( (response == NULL) || (response_buffer_length == 0) ) {
		...
		if ( debug_flag ) {
			MX_DEBUG(-2,("%s: received '%s' from '%s'",
				fname, response, radicon_xineos_gige->record->name));
		}
	}
	...
	if ( num_bytes_available == 1 ) {
		  ...
		  && ( c != '>' ) )
		{
			...
			"but it was not a LF character or a '>' character.",
				c, radicon_xineos_gige->record->name );
		}
	}
#endif
