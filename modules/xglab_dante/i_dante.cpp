			...
			const char *calling_fname )
{
	...
	*dante_mca = (MX_DANTE_MCA *) mca->record->record_type_struct;
	...
	dante_record = (*dante_mca)->handel_record;
	...
	*dante = (MX_DANTE *) handel_record->record_type_struct;
...
static mx_status_type
mxi_dante_get_mcs_array( MX_DANTE *handel,
				unsigned long num_mcs,
				 MX_MCS **mcs_array )
{
	static const char fname[] = "mxi_dante_get_mcs_array()";
	...
	if ( dante == (MX_DANTE *) NULL ) {
		return mx_error( MXE_NULL_ARGUMENT, fname,
		"The MX_DANTE pointer passed was NULL." );
	}
	...
	for ( i = 0; i < num_mcs; i++ ) {
		...
		if ( dante_mca == (MX_DANTE_MCA *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
			"The MX_DANTE_MCA pointer for MCA '%s' belonging "
			"to Handel record '%s' is NULL.",
				mca_record->name, dante->record->name );
		}

		MX_DEBUG(-2,("%s: (%lu) dante_mca = %p",
			fname, i, dante_mca));

		mcs_record = dante_mca->child_mcs_record;
...
static mx_status_type
mxi_dante_wait_for_buffers_full( MX_DANTE *handel, char buffer_name )
{
	static const char fname[] = "mxi_dante_wait_for_buffers_full()";
...
#if MXI_DANTE_DEBUG_WORK_FUNCTIONS
	MX_DEBUG(-2,("%s invoked for record '%s', buffer_name = '%c'",
		fname, dante->record->name, buffer_name ));
#endif
	...
	for ( channel = 0;
		channel < MXI_DANTE_CHANNELS_PER_MODULE;
		channel++ )
	{
	    buffer_full = 0;
...
static mx_status_type
mxi_dante_read_buffers( MX_DANTE *handel,
			unsigned long measurement_number,
			char buffer_name,
			unsigned long num_buffers,
			MX_MCS **mcs_array )
{
	static const char fname[] = "mxi_dante_read_buffers()";
	...
	MX_DEBUG(-2,("%s invoked for record '%s', measurement_number = %lu, "
		...
		fname, dante->record->name, measurement_number,
		...
		channel++ )
	{
		...
		dante_mcs = (MX_DANTE_MCS *) mcs->record->record_type_struct;

		if ( dante_mcs == (MX_DANTE_MCS *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
			"The MX_DANTE_MCS pointer for MCS '%s' is NULL.",
				mcs->record->name );
		}

		switch( buffer_name ) {
		case 'a':
			buffer_ptr = dante_mcs->buffer_a;
			break;
		case 'b':
			buffer_ptr = dante_mcs->buffer_b;
			break;
		default:
...
#if MXI_DANTE_DEBUG_MUTEX
		MX_DEBUG(-2,("%s: thread %#lx about to lock MX mutex %p",
				fname, mx_get_thread_id(NULL), dante->mutex ));
#endif

		mx_mutex_lock( dante->mutex );
		...
		MX_DEBUG(-2,("%s: MARKER 1, mutex = %p", fname, dante->mutex));

		xia_status = xiaGetRunData( channel, run_data_name, buffer_ptr);

		MX_DEBUG(-2,("%s: MARKER 2, mutex = %p", fname, dante->mutex));
...
static mx_status_type
mxi_dante_notify_buffers_read( MX_DANTE *handel, char buffer_name )
{
	static const char fname[] = "mxi_dante_notify_buffers_read()";
...
/*
 * mxi_dante_mcs_monitor_thread_fn() should only be running while an
 * MCS sequence is running.  It is started by mxd_dante_mcs_arm()
 * and exits when the MCS sequence is complete.
 */

MX_EXPORT mx_status_type
mxi_dante_mcs_monitor_thread_fn( MX_THREAD *thread, void *thread_args )
{
	static const char fname[] = "mxi_dante_mcs_monitor_thread_fn()";

	MX_DANTE *dante = NULL;

	unsigned long num_mcs = 0;

	MX_MCS *mcs_array[MXI_DANTE_CHANNELS_PER_MODULE];
	...
	if ( thread_args == NULL ) {
		...
		"dante->monitor_thread will now point to a memory block "
		"that has been freed.  However, there is no real way to do "
		"anything about this, since we were supposed to get the "
		"MX_DANTE pointer from the thread_args pointer." );
	}

	dante = (MX_DANTE *) thread_args;

#if MXI_DANTE_DEBUG_MONITOR_THREAD
	MX_DEBUG(-2,("%s invoked for XIA Handel record '%s'.",
		fname, dante->record->name ));
#endif

	/* Get an array of all the MX_MCS data structures that the MX_DANTE
	 * record knows about.
	 */

	num_mcs = dante->num_mcas;

	if ( num_mcs > MXI_DANTE_CHANNELS_PER_MODULE ) {
		...
		"only supports the first %d channels.",
			dante->record->name,
			dante->num_mcas,
			MXI_DANTE_CHANNELS_PER_MODULE );

		num_mcs = MXI_DANTE_CHANNELS_PER_MODULE;
	}

	MX_DEBUG(-2,("%s: dante = %p '%s', num_mcs = %ld",
		fname, dante, handel->record->name, num_mcs));
#if 0
		...
		fname, dante->mca_record_array));
	{
		...
		for ( n = 0; n < num_mcs; n++ ) {
			MX_DEBUG(-2,("%s: dante->mca_record_array[%d] = %p",
				fname, n, dante->mca_record_array[n]));

			if ( dante->mca_record_array[n] != NULL ) {
			 MX_DEBUG(-2,("%s: dante->mca_record_array[%d] = '%s'",
				fname, n, dante->mca_record_array[n]->name));
			}
		}
	}
#endif

	mx_status = mxi_dante_get_mcs_array( handel, num_mcs, mcs_array );
	...
	/* We need to find the MX_DANTE_MCS that started this sequence. */
...
#if MXI_DANTE_DEBUG_MONITOR_THREAD_BUFFERS
	MX_DEBUG(-2,("%s: j = %lu, WAITING for buffer_a", fname, j ));
#endif
		mx_status = mxi_dante_wait_for_buffers_full( handel, 'a' );
...
#endif
		mx_status = mxi_dante_read_buffers( handel, j,
							'a', 4, mcs_array );
	...
	MX_DEBUG(-2,("%s: j = %lu, buffer_a COPIED", fname, j ));
#endif
		mx_status = mxi_dante_notify_buffers_read( handel, 'a' );
		...
		MX_DEBUG(-2,("%s: Handel '%s' MEASUREMENT %ld COMPLETE.",
			fname, dante->record->name, j ));
#endif
		j++;

		mx_atomic_increment32( &(dante->last_pixel_number) );

		mx_atomic_increment32( &(dante->total_num_pixels) );
...
#endif
		mx_status = mxi_dante_wait_for_buffers_full( handel, 'b' );
...
#endif
		mx_status = mxi_dante_notify_buffers_read( handel, 'b' );
...
mxi_dante_create_record_structures( MX_RECORD *record )
{
	...
	dante = (MX_DANTE *) malloc( sizeof(MX_HANDEL) );

	if ( dante == (MX_DANTE *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
			"Can't allocate memory for MX_DANTE structure." );
	}
	...
	dante->last_measurement_interval = -1.0;

	dante->bypass_xia_preset_type = FALSE;

#if MXI_DANTE_DEBUG
	dante->debug_flag = TRUE;
#else
	dante->debug_flag = FALSE;
#endif
...
mxi_dante_finish_record_initialization( MX_RECORD *record )
{
	...
	dante->save_filename[0] = '\0';

	return MX_SUCCESSFUL_RESULT;
}

/* mxi_dante_set_data_available_flags() sets mca->new_data_available
 * and dante_mca->new_statistics_available for all of the MCAs controlled
...
MX_EXPORT mx_status_type
mxi_dante_set_data_available_flags( MX_DANTE *handel,
					int flag_value )
{
	static const char fname[] =
		"mxi_dante_set_data_available_flags()";
	...
	MX_DEBUG( 2,("%s invoked for '%s', flag_value = %d",
		fname, dante->record->name, flag_value));

	mca_record_array = dante->mca_record_array;

	if ( mca_record_array == (MX_RECORD **) NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"The mca_record_array pointer for 'dante' record '%s'.",
			dante->record->name );
	}
	...
	for ( i = 0; i < dante->num_mcas; i++ ) {
		...
		dante_mca->new_statistics_available = flag_value;
	}
 ...
 * the file dante->config_filename is found in.  If all of the other XIA
...
static mx_status_type
mxi_dante_set_xiahome( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_set_xiahome()";
	...
	if ( strlen( dante->config_filename ) == 0 ) {
		return MX_SUCCESSFUL_RESULT;
	}
...
static mx_status_type
mxi_dante_load_config( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_load_config()";
	...
	MX_XIA_SYNC( xiaLoadSystem( "dante_ini", handel->config_filename ) );

#if MXI_DANTE_DEBUG_TIMING
	MX_HRT_END( measurement );

	MX_HRT_RESULTS( measurement, fname,
		"xiaLoadSystem(\"dante_ini\", \"%s\")",
		dante->config_filename );
#endif
	...
	if ( dante->mca_record_array != NULL ) {
		for ( i = 0; i < dante->num_mcas; i++ ) {
			mca_record = dante->mca_record_array[i];
...
/*
 * mxi_dante_load_new_config() encapsulates all of the operations that
 * must happen when a new configuration file is loaded by writing to the
 * MX record field 'config_filename'.
 */

static mx_status_type
mxi_dante_load_new_config( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_load_new_config()";
	...
	mx_status = mxi_dante_load_config( handel );
	...
	MX_HRT_RESULTS( measurement, fname,
		"for record '%s'", dante->record->name );
#endif
	return MX_SUCCESSFUL_RESULT;
}
...
static mx_status_type
mxi_dante_save_config( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_save_config()";
	...
	MX_XIA_SYNC( xiaSaveSystem( "dante_ini", handel->save_filename ) );

	if ( xia_status != XIA_SUCCESS ) {
		return mx_error( MXE_INTERFACE_ACTION_FAILED, fname,
		"Attempt to save the Handel configuration to the file '%s' "
		"failed for MCA '%s'.  Error code = %d, '%s'",
		dante->save_filename, handel->record->name,
		xia_status, mxi_dante_strerror( xia_status ) );
	}
...
static void
mxi_dante_redirect_log_output( MX_DANTE *handel )
{
	char log_filename[MXU_FILENAME_LENGTH+1];
	char *mxdir;
	int xia_status;

	if ( strlen( dante->log_filename ) > 0 ) {
		strlcpy( log_filename, dante->log_filename,
						MXU_FILENAME_LENGTH );
	} else {
		mxdir = getenv("MXDIR");
		...
		if ( mxdir == NULL ) {
			strlcpy( log_filename,
				"/opt/mx/log/dante.log",
				sizeof(log_filename)  );
		} else {
			snprintf( log_filename, sizeof(log_filename),
				"%s/log/dante.log", mxdir );
		}
	}
...
mxi_dante_open( MX_RECORD *record )
{
	...
	if ( dante->handel_flags & MXF_DANTE_BREAKPOINT_AT_STARTUP ) {
		mx_breakpoint();
	}

	/* Set up the MX Handel mutex. */

	mx_status = mx_mutex_create( &(dante->mutex) );
	...
	display_config = dante->handel_flags &
	...
	os_status = access( dante->config_filename, R_OK );
	...
	if (dante->handel_flags & MXF_DANTE_WRITE_LOG_OUTPUT_TO_FILE)
	{
		mxi_dante_redirect_log_output( handel );
	}

	/* Set the logging level. */

	if ( dante->handel_log_level < MD_ERROR ) {
		dante->handel_log_level = MD_ERROR;
	}
	if ( dante->handel_log_level > MD_DEBUG ) {
		dante->handel_log_level = MD_DEBUG;
	}

	MX_XIA_SYNC( xiaSetLogLevel( dante->handel_log_level ) );

	if ( xia_status != XIA_SUCCESS ) {
		return mx_error( MXE_INTERFACE_ACTION_FAILED, fname,
		"An attempt to set the Handel log level to %lu failed.  "
		"Error code = %d, '%s'", dante->handel_log_level,
			xia_status, mxi_dante_strerror( xia_status ) );
	}
	...
	if ( num_items != 3 ) {
		...
		dante->handel_version = 0;
	} else {
		dante->handel_version = 1000000L*major + 1000L*minor + update;
	}

	/* If requested, set the XIAHOME environment variable to the
	 * basename of the dante->config_filename variable.
	 */

	if ( dante->handel_flags & MXF_DANTE_CREATE_XIAHOME_VARIABLE ) {
		mx_status = mxi_dante_set_xiahome( handel );
	...
	dante->num_detectors = num_detectors;
	...
	dante->num_mcas = total_num_mcas;

	/* Allocate and initialize an array to store pointers to all of
	 * the MCA records used by this interface.
	 */

	dante->mca_record_array = ( MX_RECORD ** ) malloc(
				dante->num_mcas * sizeof( MX_RECORD * ) );
	...
	dante->num_modules = num_modules;

	/* Allocate a two-dimensional module pointer array. */

	dante->mcas_per_module = 4;	/* FIXME: For the DXP-XMAP. */

	dimension_array[0] = dante->num_modules;
	dimension_array[1] = dante->mcas_per_module;

	size_array[0] = sizeof(MX_RECORD *);
	size_array[1] = sizeof(MX_RECORD **);

	dante->module_array = mx_allocate_array( MXFT_RECORD,
	...
	if ( dante->module_array == NULL ) {
		...
		"module array for Handel interface '%s'.",
			dante->num_modules, handel->mcas_per_module,
			record->name );
	}

	for ( i = 0; i < dante->num_modules; i++ ) {
		for ( j = 0; j < dante->mcas_per_module; j++ ) {
			dante->module_array[i][j] = NULL;
		}
	}

	dante->use_module_statistics_2 = TRUE;

	/* Initialize atomic counter variables. */

	mx_atomic_write32( &(dante->last_pixel_number), -1 );
	mx_atomic_write32( &(dante->total_num_pixels), 0 );
	mx_atomic_write32( &(dante->total_num_pixels_at_start), 0 );

#if 1
	MX_DEBUG(-2,("%s: last_pixel_number = %d, total_num_pixels = %d, "
	"total_num_pixels_at_start = %d",
		fname, (int) dante->last_pixel_number,
		(int) dante->total_num_pixels,
		(int) dante->total_num_pixels_at_start ));
#endif
...
MX_EXPORT mx_status_type
mxi_dante_set_acquisition_values_for_all_channels( MX_DANTE *handel,
						char *value_name,
						double *value_ptr,
						mx_bool_type apply_flag )
{
	...
	if ( dante->debug_flag ) {
		MX_DEBUG(-2,("%s: setting acquisition value '%s' for "
			...
			fname, value_name, dante->record->name, *value_ptr ));
	}
	...
	if ( apply_flag ) {
		mx_status = mxi_dante_apply_to_all_channels( handel );
...
MX_EXPORT mx_status_type
mxi_dante_apply_to_all_channels( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_apply_to_all_channels()";

	int m, xia_status, ignored;
	MX_RECORD *first_mca_record;
	MX_DANTE_MCA *first_dante_mca;
	...
	for ( m = 0; m < dante->num_modules; m++ ) {
		/* Find the first MCA for each module and then
		 * do an apply to it.
		 */

		first_mca_record = dante->module_array[m][0];

		if ( first_mca_record == NULL ) {
			continue;  /* Go back to the top of the loop. */
		}

		first_dante_mca = first_mca_record->record_type_struct;

		if ( first_dante_mca == NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE,
			fname, "The MX_DANTE_MCA pointer for Handel "
			"MCA '%s' is NULL.",
		...
		MX_XIA_SYNC( xiaBoardOperation(
				first_dante_mca->detector_channel,
				"apply", (void *) &ignored ) );
...
MX_EXPORT mx_status_type
mxi_dante_read_parameter( MX_MCA *mca,
			char *parameter_name,
			unsigned long *value_ptr )
{
	static const char fname[] = "mxi_dante_read_parameter()";
	...
	mx_status = mxi_dante_get_pointers( mca,
					&dante_mca, &handel, fname );
	...
	if ( dante_mca->debug_flag ) {
		...
		("%s: Record '%s', channel %ld, parameter '%s'.",
			...
			dante_mca->detector_channel, parameter_name));
	}
	...
	MX_XIA_SYNC( xiaGetParameter( dante_mca->detector_channel,
	...
	if ( xia_status != XIA_SUCCESS ) {
		...
		"Error code = %d, '%s'",
			parameter_name, dante_mca->detector_channel,
...
MX_EXPORT mx_status_type
mxi_dante_write_parameter( MX_MCA *mca,
			char *parameter_name,
			unsigned long value )
{
	static const char fname[] = "mxi_dante_write_parameter()";
	...
	if ( dante_mca->debug_flag ) {
		...
		("%s: Record '%s', channel %ld, parameter '%s', value = %lu.",
			fname, mca->record->name, dante_mca->detector_channel,
	...
	MX_XIA_SYNC( xiaSetParameter( dante_mca->detector_channel,
					parameter_name, short_value ) );
...
MX_EXPORT mx_status_type
mxi_dante_write_parameter_to_all_channels( MX_DANTE *handel,
					char *parameter_name,
					unsigned long value )
{
	static const char fname[] =
		"mxi_dante_write_parameter_to_all_channels()";
...
mxi_dante_special_processing_setup( MX_RECORD *record )
{
	...
	for ( i = 0; i < record->num_record_fields; i++ ) {
		...
		case MXLV_DANTE_ACQUISITION_VALUE_NAME:
		case MXLV_DANTE_CONFIG_FILENAME:
		case MXLV_DANTE_MAPPING_MODE:
		case MXLV_DANTE_MAPPING_PIXEL_NEXT:
		case MXLV_DANTE_PARAMETER_NAME:
		case MXLV_DANTE_PIXEL_ADVANCE_MODE:
		case MXLV_DANTE_SAVE_FILENAME:
		case MXLV_DANTE_SYNC_COUNT:
...
MX_EXPORT mx_status_type
mxi_dante_reallocate_buffers( MX_DANTE *handel,
				size_t buffer_size )
{
	static const char fname[] = "mxi_dante_reallocate_buffers()";
	...
	if (1) {
	    ...
	    MX_DEBUG(-2,("%s: dante->num_mcas = %ld",
				fname, dante->num_mcas));

	    MX_DEBUG(-2,("%s: *** MARKER BEFORE ***",fname));

#if 1
	    mx_status = mxi_dante_get_mcs_array( handel,
						dante->num_mcas,
						mcs_array );
		...
		channel++ )
	{
		...
		mx_free( dante_mcs->buffer_a );
		mx_free( dante_mcs->buffer_b );

		/* Allocate the new buffers. */

		dante_mcs->buffer_a = malloc( handel_mcs->buffer_length
						* sizeof(long) );

		if ( dante_mcs->buffer_a == (uint32_t *) NULL ) {
			return mx_error( MXE_OUT_OF_MEMORY, fname,
			"Ran out of memory trying to allocate a "
			"%lu element array of unsigned longs for "
			"'buffer_a' belonging to XIA MCS '%s'.",
				dante_mcs->buffer_length,
				mcs->record->name );
		}

		dante_mcs->buffer_b = malloc( handel_mcs->buffer_length
						* sizeof(long) );

		if ( dante_mcs->buffer_b == (uint32_t *) NULL ) {
			return mx_error( MXE_OUT_OF_MEMORY, fname,
			"Ran out of memory trying to allocate a "
			"%lu element array of unsigned longs for "
...
MX_EXPORT mx_status_type
mxi_dante_start_run( MX_DANTE *handel,
			mx_bool_type resume_run )
{
	static const char fname[] = "mxi_dante_start_run()";
	...
	if ( dante->debug_flag ) {
		MX_DEBUG(-2,("%s for Handel interface '%s', resume_run = %d",
			fname, dante->record->name, (int) resume_run));
	}
	...
	for ( m = 0; m < dante->num_modules; m++ ) {
		...
		if ( first_dante_mca == (MX_DANTE_MCA *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		    "The MX_DANTE_MCA pointer for Handel MCA '%s' is NULL.",
				first_mca_record->name );
		}
		...
		MX_XIA_SYNC( xiaStartRun( first_dante_mca->detector_channel,
							resume_flag ) );
...
MX_EXPORT mx_status_type
mxi_dante_stop_run( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_stop_run()";
	...
	for ( m = 0; m < dante->num_modules; m++ ) {
		...
		MX_XIA_SYNC( xiaStopRun( first_dante_mca->detector_channel ) );
...
MX_EXPORT mx_status_type
mxi_dante_is_busy( MX_DANTE *handel,
			mx_bool_type *busy )
{
	static const char fname[] = "mxi_dante_is_busy()";
	...
	*busy = FALSE;
	...
	for ( i = 0; i < dante->num_mcas; i++ ) {
		...
		dante_mca = mca_record->record_type_struct;
...
MX_EXPORT mx_status_type
mxi_dante_set_preset( MX_DANTE *handel,
			double preset_type,
			double preset_value )
{
	static const char fname[] = "mxi_dante_set_preset()";
...
MX_EXPORT mx_status_type
mxi_dante_show_parameter( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_show_parameter()";
	...
	mx_info( "-------------------------------------------------------" );
	mx_info( "Parameter '%s' values:", dante->parameter_name );
	mx_info( " " );
	...
	for ( i = 0; i < dante->num_mcas; i++ ) {
		...
		if ( dante_mca == (MX_DANTE_MCA *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
			"The MX_DANTE_MCA pointer for MCA '%s' is NULL.",
				mca_record->name );
		}

		MX_XIA_SYNC( xiaGetParameter( dante_mca->detector_channel,
						dante->parameter_name,
						&parameter_value ) );
...
MX_EXPORT mx_status_type
mxi_dante_show_acquisition_value( MX_DANTE *handel )
{
	static const char fname[] = "mxi_dante_show_acquisition_value()";
...
MX_EXPORT mx_status_type
mxi_dante_get_acq_value_as_long( MX_DANTE *handel,
				char *value_name,
				long *acq_value,
				mx_bool_type apply_flag )
{
	static const char fname[] = "mxi_dante_get_acq_value_as_long()";
...
MX_EXPORT mx_status_type
mxi_dante_set_acq_value_as_long( MX_DANTE *handel,
				char *value_name,
				long acq_value,
				mx_bool_type apply_flag )
{
	static const char fname[] = "mxi_dante_set_acq_value_as_long()";
...
const char *
mxi_dante_strerror( int handel_status )
{
	static struct {
		int dante_error_code;
		const char dante_error_message[140];
	} error_code_table[] = {
		{   0, "XIA_SUCCESS" },
		{   1, "XIA_OPEN_FILE" },
		{   2, "XIA_FILEERR" },
...
"An attempt was made to set the gaindac on a board that doesn't have a VGA" },
	...
	for ( i = 0; i < num_error_codes; i++ ) {
		...
		if ( dante_status == error_code_table[i].handel_error_code )
	...
	return &( error_code_table[i].dante_error_message[0] );
}
			...
			int operation )
{
...
#if 0
		case MXLV_DANTE_MAPPING_MODE:
			mx_status = mxi_dante_get_acq_value_as_long( handel,
							"mapping_mode",
							&(dante->mapping_mode),
							TRUE );
			break;
	...
	case MX_PROCESS_PUT:
		switch( record_field->label_value ) {
		case MXLV_DANTE_CONFIG_FILENAME:
			mx_status = mxi_dante_load_new_config( handel );

			break;
		case MXLV_DANTE_SAVE_FILENAME:
			mx_status = mxi_dante_save_config( handel );

			break;
		case MXLV_DANTE_PARAMETER_NAME:
			mx_status = mxi_dante_show_parameter( handel );
			break;
		case MXLV_DANTE_ACQUISITION_VALUE_NAME:
			mx_status = mxi_dante_show_acquisition_value(handel);
			break;
		case MXLV_DANTE_MAPPING_MODE:
			mx_status = mxi_dante_set_acq_value_as_long( handel,
							"mapping_mode",
							dante->mapping_mode,
							TRUE );
			break;
...
#if MXI_DANTE_DEBUG_MAPPING_PIXEL_NEXT
			MX_DEBUG(-2,
