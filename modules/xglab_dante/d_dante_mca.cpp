...
#if MXD_DANTE_MCA_DEBUG_STATISTICS
#   define DANTE_MCA_DEBUG_STATISTICS(x) \
...
mxd_dante_mca_create_record_structures( MX_RECORD *record )
{
	...
	if ( dante_mca == (MX_DANTE_MCA *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Can't allocate memory for MX_DANTE_MCA structure." );
	}
	...
	dante_mca->adc_trace_step_size = 5000.0;	/* in nanoseconds */
	dante_mca->adc_trace_length = 0;
	dante_mca->adc_trace_array = NULL;

	dante_mca->baseline_history_length = 0;
	dante_mca->baseline_history_array = NULL;

	dante_mca->mca_record_array_index = -1;

	dante_mca->firmware_code_variant = (unsigned long) -1;
	dante_mca->firmware_code_revision = (unsigned long) -1;
	dante_mca->hardware_scas_are_enabled = FALSE;

	dante_mca->has_mapping_firmware = FALSE;

	for ( i = 0; i < MX_DANTE_MCA_MAX_SCAS; i++ ) {
		dante_mca->sca_has_been_initialized[i] = FALSE;
	}

	dante_mca->acquisition_value_name[0] = '\0';
	dante_mca->acquisition_value = 0;
	dante_mca->acquisition_value_to_all = 0;
	dante_mca->apply_flag = FALSE;
	dante_mca->apply_to_all = FALSE;

	dante_mca->use_double_roi_integral_array = FALSE;
	dante_mca->double_roi_integral_array = NULL;

#if MXD_DANTE_MCA_DEBUG
	dante_mca->debug_flag = TRUE;
#else
	dante_mca->debug_flag = FALSE;
#endif
...
mxd_dante_mca_finish_record_initialization( MX_RECORD *record )
{
	...
	if ( mca->maximum_num_rois > MX_DANTE_MCA_MAX_SCAS ) {
...
"than the maximum allowed value of %d.",
	...
	dante_mca->new_statistics_available = TRUE;

	DANTE_MCA_DEBUG_STATISTICS( dante_mca );
...
static mx_status_type
mxd_dante_mca_dante_open( MX_MCA *mca,
			MX_DANTE_MCA *dante_mca,
			MX_RECORD *dante_record )
{
	static const char fname[] = "mxd_dante_mca_dante_open()";
	...
	"MX database file _after_ the entry for '%s'.",
			dante_record->name, mca->record->name,
			mca->record->name, dante_record->name );
	}

	display_config = dante->dante_flags &
			MXF_DANTE_DISPLAY_CONFIGURATION_AT_STARTUP;

	/* Allocate memory for the detector and module aliases. */

	dante_mca->detector_alias = (char *)
			malloc( (MAXALIAS_LEN+1) * sizeof(char) );

	if ( dante_mca->detector_alias == NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Cannot allocate a module alias string of %d chars.",
			MAXALIAS_LEN+1 );
	}

	dante_mca->module_alias = (char *)
			malloc( (MAXALIAS_LEN+1) * sizeof(char) );

	if ( dante_mca->module_alias == NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Cannot allocate a module alias string of %d chars.",
			MAXALIAS_LEN+1 );
	}

	dante_mca->module_type = (char *)
			malloc( (MAXALIAS_LEN+1) * sizeof(char) );

	if ( dante_mca->module_type == NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
	...
	*ptr = '\0';
	...
	dante_mca->detector_channel = mx_string_to_long( ptr );
	...
	num_items = sscanf( mca_label_copy,
				mca_label_format,
				dante_mca->module_alias,
				dante_mca->detector_alias );
	...
	if ( num_items != 2 ) {
		...
		"\"module1 detector1:0\".",
			dante_mca->mca_label, mca->record->name );
	}

	num_items = sscanf( dante_mca->module_alias,
				"module%lu", &(dante_mca->module_number) );

	if ( num_items != 1 ) {
		return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
		"Could not get the module number from the module alias '%s' "
		"for MCA '%s'.",  dante_mca->module_alias,
	...
	if ( display_config ) {
		...
		"detector alias = '%s', detector channel = %ld",
			dante_mca->record->name,
			dante_mca->module_alias,
			dante_mca->module_number,
			dante_mca->detector_alias,
			dante_mca->detector_channel );
	}
	...
	MX_XIA_SYNC( xiaGetModuleItem( dante_mca->module_alias,
					"module_type",
					dante_mca->module_type ) );
	...
	module_channel = dante_mca->detector_channel % dante->mcas_per_module;
	...
	if ( detector_channel_alt != dante_mca->detector_channel ) {
	...
	MX_XIA_SYNC( xiaStopRun( dante_mca->detector_channel ) );

	if ( xia_status != XIA_SUCCESS ) {
		return mx_error( MXE_DEVICE_ACTION_FAILED, fname,
		"Cannot stop data acquisition for XIA DXP MCA '%s'.  "
		"Error code = %d, '%s'", dante_mca->record->name,
			xia_status, mxi_dante_strerror( xia_status ) );
	}
	...
	if ( mca->maximum_num_rois > 0 ) {
		dante_mca->hardware_scas_are_enabled = TRUE;
	} else {
		dante_mca->hardware_scas_are_enabled = FALSE;
	}
	...
	for ( i = 0; i < dante->num_mcas; i++ ) {
		...
		if ( dante->mca_record_array[i] == NULL ) {
			...
			dante_mca->dxp_module = -1;
			dante_mca->crate = -1;
			dante_mca->slot = -1;
			dante_mca->dxp_channel = -1;
...
#if MXD_DANTE_MCA_DEBUG_MCA_RECORD_ARRAY
	MX_DEBUG(-2,("%s: dante_mca = '%s', dante_mca->record = %p",
		fname, dante_mca->record->name, dante_mca->record));
	MX_DEBUG(-2,("%s: dante->mca_record_array = %p",
		fname, dante->mca_record_array));
	MX_DEBUG(-2,("%s: dante->mca_record_array[%lu] = %p",
		fname, i, dante->mca_record_array[i] ));
#endif

	/* Search for an empty slot in the modules array. */

	if ( ( dante_mca->module_number < 1 )
	  || ( dante_mca->module_number > dante->num_modules ) )
	{
		return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
	...
	if ( dante->module_array == NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"The module_array pointer for Handel interface '%s' "
		"has not been initialized.", dante->record->name );
	}

	i = dante_mca->module_number - 1;

	for ( j = 0; j < dante->mcas_per_module; j++ ) {
		if ( dante->module_array[i][j] == NULL ) {
			dante->module_array[i][j] = mca->record;
			break;
		}
	}

	if ( j >= dante->mcas_per_module ) {
		return mx_error( MXE_WOULD_EXCEED_LIMIT, fname,
		"Handel record '%s', module number %lu has too many "
		"records (%lu).  The maximum allowed is %ld.",
			dante->record->name,
			dante_mca->module_number, j,
			dante->mcas_per_module );
	}

#if MXD_DANTE_MCA_DEBUG_MCA_RECORD_ARRAY
	MX_DEBUG(-2,("%s: dante_mca->module_number = %lu",
		fname, dante_mca->module_number));
	MX_DEBUG(-2,("%s: dante->module_array[%lu][%lu] = %p",
		fname, i, j, dante->module_array[i][j]));
#endif

	/* See how many bins are in the spectrum. */

	MX_XIA_SYNC( xiaGetRunData( dante_mca->detector_channel,
	...
	dante_mca->num_spectrum_bins = (unsigned int) ulong_value;

	if ( dante_mca->debug_flag ) {
		MX_DEBUG(-2,("%s: MCA '%s': num_spectrum_bins = %u",
		fname, mca->record->name, dante_mca->num_spectrum_bins));
	}
	...
	if ( ( sizeof(unsigned long) == 4 )
	  && ( mca->maximum_num_channels >= dante_mca->num_spectrum_bins ) )
	{
		 ...
		 * dante_mca->spectrum_array to mca->channel_array.
		 */

		dante_mca->use_mca_channel_array = TRUE;

		dante_mca->spectrum_array = NULL;
	} else {
		/* We _do_ need to allocate a separate spectrum array.
		 * This will happen for one of two reasons:
		 *
		 * 1.  Either the value dante_mca->num_spectrum_bins
		...
		dante_mca->use_mca_channel_array = FALSE;

		dante_mca->spectrum_array = (unsigned long *) malloc(
		    dante_mca->num_spectrum_bins * sizeof(unsigned long) );

		if ( dante_mca->spectrum_array == (unsigned long *) NULL ) {
			return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Cannot allocated a DXP spectrum array of %u unsigned longs",
			dante_mca->num_spectrum_bins );
		}
	}
	...
	dante_mca->baseline_length = (unsigned int) ulong_value;

	dante_mca->baseline_array = (unsigned long *)
	    malloc( dante_mca->baseline_length * sizeof(unsigned long) );

	if ( dante_mca->baseline_array == (unsigned long *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Cannot allocate a DXP baseline array of %lu unsigned longs",
			dante_mca->baseline_length );
	}
...
mxd_dante_mca_open( MX_RECORD *record )
{
	...
	mx_status = mxd_dante_mca_dante_open(mca, dante_mca, dante->record);
	...
	MX_XIA_SYNC( xiaGetAcquisitionValues( dante_mca->detector_channel,
					"mapping_mode", &mapping_mode ) );

	switch( xia_status ) {
	case XIA_SUCCESS:
		dante_mca->has_mapping_firmware = TRUE;
		break;
	default:
		dante_mca->has_mapping_firmware = FALSE;
		break;
	...
	MX_DEBUG(-2,("%s: MCA '%s' has_mapping_firmware = %d",
		fname, record->name, (int) dante_mca->has_mapping_firmware));
#endif

#if 0
	/* Find out what firmware variant and revision is used by this MCA. */

	mx_status = mxi_dante_read_parameter( mca, "CODEVAR", &codevar );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	dante_mca->firmware_code_variant = (int) codevar;

	mx_status = mxi_dante_read_parameter( mca, "CODEREV", &coderev );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	dante_mca->firmware_code_revision = (int) coderev;
#endif
	...
	if ( dante_mca->hardware_scas_are_enabled ) {
#if 0
...
#endif
		/* The call above to xiaSteAcquisitionValues() implicitly
	...
	xia_status, mxi_dante_strerror( xia_status ) );
		...
		if ( strcmp( dante_mca->module_type, "xmap" ) == 0 ) {
			dante_mca->use_double_roi_integral_array = TRUE;
		}

#if MXD_DANTE_MCA_DEBUG_DOUBLE_ROIS
		MX_DEBUG(-2,("%s: MCA '%s' use_double_roi_integral_array = %d",
			fname, mca->record->name,
			(int) dante_mca->use_double_roi_integral_array ));
#endif

		if( dante_mca->use_double_roi_integral_array == TRUE ) {

			dante_mca->double_roi_integral_array = (double *)
			    malloc( mca->maximum_num_rois * sizeof(double) );

			if ( dante_mca->double_roi_integral_array == NULL ) {
				return mx_error( MXE_OUT_OF_MEMORY, fname,
		...
		MX_DEBUG(-2,("%s: MCA '%s' double_roi_integral_array = %p",
			fname, mca->record->name,
			dante_mca->double_roi_integral_array));
#endif
	}
	...
	"MCA '%s': codevar = %#lx, coderev = %#lx, hardware SCAs = %d",
			dante_mca->record->name,
			dante_mca->firmware_code_variant,
			dante_mca->firmware_code_revision,
			(int) dante_mca->hardware_scas_are_enabled );
	}
#endif
	...
	dante_mca->old_preset_type = (unsigned long) ULONG_MAX;
	dante_mca->old_preset_time = DBL_MAX;

#if MXD_DANTE_MCA_DEBUG
	MX_DEBUG(-2,("%s: MCA '%s', dante_mca->hardware_scas_are_enabled = %d",
		fname, record->name,
		(int) dante_mca->hardware_scas_are_enabled));
#endif
...
mxd_dante_mca_close( MX_RECORD *record )
{
	...
	if ( dante_mca->baseline_array != NULL ) {
		mx_free( dante_mca->baseline_array );
	}
	if ( dante_mca->spectrum_array != NULL ) {
		mx_free( dante_mca->spectrum_array );
	}
...
mxd_dante_mca_special_processing_setup( MX_RECORD *record )
{
	...
	for ( i = 0; i < record->num_record_fields; i++ ) {
		...
		case MXLV_DANTE_MCA_ACQUISITION_VALUE:
		case MXLV_DANTE_MCA_ACQUISITION_VALUE_TO_ALL:
		case MXLV_DANTE_MCA_APPLY:
		case MXLV_DANTE_MCA_APPLY_TO_ALL:
		case MXLV_DANTE_MCA_ADC_TRACE_ARRAY:
		case MXLV_DANTE_MCA_BASELINE_ARRAY:
		case MXLV_DANTE_MCA_BASELINE_HISTORY_ARRAY:
		case MXLV_DANTE_MCA_GAIN_CHANGE:
		case MXLV_DANTE_MCA_GAIN_CALIBRATION:
		case MXLV_DANTE_MCA_PARAMETER_VALUE:
		case MXLV_DANTE_MCA_PARAM_VALUE_TO_ALL_CHANNELS:
		case MXLV_DANTE_MCA_SHOW_PARAMETERS:
		case MXLV_DANTE_MCA_SHOW_ACQUISITION_VALUES:
		case MXLV_DANTE_MCA_STATISTICS:
...
mxd_dante_mca_arm( MX_MCA *mca )
{
	...
	if ( dante_mca->has_mapping_firmware ) {
	...
	case MXF_MCA_PRESET_NONE:
		preset_type = MXF_DANTE_MCA_PRESET_NONE;
		preset_time = 0.0;
		break;

	case MXF_MCA_PRESET_LIVE_TIME:
		preset_type = MXF_DANTE_MCA_PRESET_LIVE_TIME;
		preset_time = mca->preset_live_time;
		break;

	case MXF_MCA_PRESET_REAL_TIME:
		preset_type = MXF_DANTE_MCA_PRESET_REAL_TIME;
		preset_time = mca->preset_real_time;
		break;

	case MXF_MCA_PRESET_COUNT:
		preset_type = MXF_DANTE_MCA_PRESET_OUTPUT_EVENTS;
	...
	dante->last_measurement_interval = preset_time;
	...
	if ( ( preset_type == dante_mca->old_preset_type )
	  && ( preset_time == dante_mca->old_preset_time ) )
	{
		MX_DEBUG( 2,("%s: Presets will NOT be changed.", fname));

		mx_status = mxd_dante_mca_start_run( mca, TRUE );
	...
	if ( dante->bypass_xia_preset_type == FALSE ) {
		preset_type_as_double = preset_type;

		mx_status = mxi_dante_set_acquisition_values_for_all_channels(
			dante, "preset_type", &preset_type_as_double, TRUE );

		if ( mx_status.code != MXE_SUCCESS )
			return mx_status;
	}

	dante_mca->old_preset_type = preset_type;

	/******* Set the timer preset. *******/

	mx_status = mxi_dante_set_acquisition_values_for_all_channels( dante,
			"preset_value", &preset_time, TRUE );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	dante_mca->old_preset_time = preset_time;
	...
	if ( mca->trigger_mode == MXF_DEV_EXTERNAL_TRIGGER ) {
		...
		mx_status = mxd_dante_mca_start_run( mca, 1 );
	}
...
mxd_dante_mca_stop( MX_MCA *mca )
{
	...
	mx_status = mxd_dante_mca_stop_run( mca );
...
mxd_dante_mca_read( MX_MCA *mca )
{
	...
	if ( dante_mca->use_mca_channel_array ) {
		array_ptr = mca->channel_array;
	} else {
		array_ptr = dante_mca->spectrum_array;
	}
...
#if 0
	mask = XIA_RUN_HARDWARE | XIA_RUN_DANTE | XIA_RUN_CT;
#else
...
mxd_dante_mca_get_parameter( MX_MCA *mca )
{
	...
	case MXLV_MCA_CURRENT_NUM_CHANNELS:
		mx_status = mxd_dante_mca_get_acquisition_values( mca,
	...
	case MXLV_MCA_PRESET_TYPE:
		if ( dante->bypass_xia_preset_type ) {
			break;
		}
		...
		dante_preset_type = mx_round( acquisition_value );

		switch( dante_preset_type ) {
		case MXF_DANTE_MCA_PRESET_NONE:
			mca->preset_type = MXF_MCA_PRESET_NONE;
			break;
		case MXF_DANTE_MCA_PRESET_REAL_TIME:
			mca->preset_type = MXF_MCA_PRESET_REAL_TIME;
			break;
		case MXF_DANTE_MCA_PRESET_LIVE_TIME:
			mca->preset_type = MXF_MCA_PRESET_LIVE_TIME;
			break;
		case MXF_DANTE_MCA_PRESET_OUTPUT_EVENTS:
			mca->preset_type = MXF_MCA_PRESET_COUNT;
			break;
		case MXF_DANTE_MCA_PRESET_INPUT_COUNTS:
			return mx_error( MXE_UNSUPPORTED, fname,
	...
	case MXLV_MCA_PRESET_REAL_TIME:
		if ( mca->preset_type != MXF_MCA_PRESET_REAL_TIME ) {
			...
			mx_status = mxd_dante_mca_busy( mca );
			...
			if ( mca->busy ) {
				...
				mx_status = mxd_dante_mca_set_parameter( mca );
				...
				mx_status =
				    mxd_dante_mca_get_acquisition_values( mca,
	...
	case MXLV_MCA_ROI_ARRAY:
		for ( i = 0; i < mca->current_num_rois; i++ ) {
		    if ( dante_mca->sca_has_been_initialized[i] == FALSE ) {
	...
	case MXLV_MCA_ROI_INTEGRAL:
		...
		if ( dante_mca->hardware_scas_are_enabled ) {
			...
			MX_DEBUG(-2,("%s: use_double_roi_integral_array = %d",
			  fname, (int) dante_mca->double_roi_integral_array));
#endif
			if ( dante_mca->use_double_roi_integral_array ) {
				integral_array =
					dante_mca->double_roi_integral_array;
			} else {
			...
			if ( dante_mca->use_double_roi_integral_array ) {
				for ( i = 0; i < mca->maximum_num_rois; i++ ) {
					dbl_value =
				    dante_mca->double_roi_integral_array[i];
...
mxd_dante_mca_set_parameter( MX_MCA *mca )
{
	...
	case MXLV_MCA_CURRENT_NUM_CHANNELS:
		...
		mx_status = mxd_dante_mca_set_acquisition_values( mca,
				"number_mca_channels", &acquisition_value,
				TRUE );
		break;
	...
	case MXLV_MCA_PRESET_TYPE:
		...
		switch( mca->preset_type ) {
		case MXF_MCA_PRESET_NONE:
			dante_preset_type = MXF_DANTE_MCA_PRESET_NONE;
			break;
		case MXF_MCA_PRESET_REAL_TIME:
			dante_preset_type = MXF_DANTE_MCA_PRESET_REAL_TIME;
			break;
		case MXF_MCA_PRESET_LIVE_TIME:
			dante_preset_type = MXF_DANTE_MCA_PRESET_LIVE_TIME;
			break;
		case MXF_MCA_PRESET_COUNT:
			dante_preset_type =
					MXF_DANTE_MCA_PRESET_OUTPUT_EVENTS;
			break;
		default:
		...
		acquisition_value = (double) dante_preset_type;
	...
	case MXLV_MCA_ROI_ARRAY:
		for ( i = 0; i < mca->current_num_rois; i++ ) {
			...
			dante_mca->sca_has_been_initialized[i] = TRUE;
		}

		mx_status = mxi_dante_apply_to_all_channels( dante );
			...
			double *corrected_roi_value )
{
	...
	mx_status = mxd_dante_mca_read_statistics( mca );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

	multiplier = mx_divide_safely( dante_mca->input_count_rate,
					dante_mca->output_count_rate );
...
#define MXU_DANTE_MCA_NUM_STATISTICS	36    /* FIXME: 36 is for DXP-XMAP */

MX_EXPORT mx_status_type
mxd_dante_mca_read_statistics( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_read_statistics()";

	MX_DANTE_MCA *dante_mca;
	MX_DANTE *dante;
	double module_statistics[MXU_DANTE_MCA_NUM_STATISTICS];
	long channel_offset;
	...
	} else if ( dante_mca->new_statistics_available ) {
		read_statistics = TRUE;
	...
	if ( dante->use_module_statistics_2 ) {
		mx_status = mxd_dante_mca_get_run_data( mca,
					"module_statistics_2",
	...
	if ( strcmp( dante_mca->module_type, "xmap" ) == 0 ) {
		...
		if ( dante_mca->debug_flag ) {
			...
			for ( i = 0; i < MXU_DANTE_MCA_NUM_STATISTICS; i++ ) {
				MX_DEBUG(-2,
				("%s: statistics[%d] = %g",
	...
	"icr = %g, ocr = %g, #under = %lu, #over = %lu", fname,
			...
			mca->live_time,
			dante_mca->energy_live_time,
			dante_mca->num_triggers,
			dante_mca->num_events,
			dante_mca->input_count_rate,
			dante_mca->output_count_rate,
			dante_mca->num_underflows,
			dante_mca->num_overflows));
	}

	if ( mca->busy == FALSE ) {
		dante_mca->new_statistics_available = FALSE;
	}
...
MX_EXPORT mx_status_type
mxd_dante_mca_get_run_data( MX_MCA *mca,
			char *name,
			void *value_ptr )
{
	static const char fname[] = "mxd_dante_mca_get_run_data()";
...
MX_EXPORT mx_status_type
mxd_dante_mca_get_acquisition_values( MX_MCA *mca,
				char *value_name,
				double *value_ptr )
{
	static const char fname[] = "mxd_dante_mca_get_acquisition_values()";
...
MX_EXPORT mx_status_type
mxd_dante_mca_set_acquisition_values( MX_MCA *mca,
				char *value_name,
				double *value_ptr,
				mx_bool_type apply_flag )
{
	static const char fname[] = "mxd_dante_mca_set_acquisition_values()";
	...
	MX_XIA_SYNC( xiaSetAcquisitionValues( dante_mca->detector_channel,
					value_name, (void *) value_ptr ) );
...
MX_EXPORT mx_status_type
mxd_dante_mca_apply( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_apply()";
	...
	MX_XIA_SYNC( xiaBoardOperation( dante_mca->detector_channel,
						"apply", (void *) &ignored ) );
...
MX_EXPORT mx_status_type
mxd_dante_mca_start_run( MX_MCA *mca, mx_bool_type clear_flag )
{
	static const char fname[] = "mxd_dante_mca_start_run()";
	...
	mxi_dante_set_data_available_flags( dante, TRUE );
	...
	MX_XIA_SYNC( xiaStartRun( dante_mca->detector_channel, resume_flag ) );
...
MX_EXPORT mx_status_type
mxd_dante_mca_stop_run( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_stop_run()";
...
MX_EXPORT mx_status_type
mxd_dante_mca_get_baseline_array( MX_MCA *mca )
{
	static const char fname[] =
			"mxd_dante_mca_get_baseline_array()";
	...
	if ( dante_mca->baseline_array == NULL ) {
	    ...
	    dante_mca->baseline_length = baseline_length;
	} else
	if ( baseline_length > dante_mca->baseline_length ) {
	    dante_mca->baseline_array = (unsigned long *)
		realloc( dante_mca->baseline_array,
	...
	void_ptr = (void *) &(dante_mca->baseline_array[0]);
	...
	field->dimension[0] = dante_mca->baseline_length;

	return MX_SUCCESSFUL_RESULT;
}

MX_EXPORT mx_status_type
mxd_dante_mca_set_gain_change( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_set_gain_change()";
	...
	MX_DEBUG(-2,("%s invoked for MCA '%s', gain_change = %g",
		fname, mca->record->name, dante_mca->gain_change));

	MX_XIA_SYNC( xiaGainOperation( dante_mca->detector_channel,
					"calibrate",
					&(dante_mca->gain_change) ) );
...
MX_EXPORT mx_status_type
mxd_dante_mca_set_gain_calibration( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_set_gain_calibration()";
	...
	MX_DEBUG(-2,("%s invoked for MCA '%s', gain_calibration = %g",
		fname, mca->record->name, dante_mca->gain_calibration));

	MX_XIA_SYNC( xiaGainCalibrate( dante_mca->detector_channel,
					dante_mca->gain_calibration ) );
...
MX_EXPORT mx_status_type
mxd_dante_mca_get_adc_trace_array( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_get_adc_trace_array()";
	...
	MX_XIA_SYNC( xiaGetSpecialRunData( dante_mca->detector_channel,
	...
	if ( dante_mca->adc_trace_array == NULL ) {
		dante_mca->adc_trace_array = (unsigned long *)
			malloc( adc_trace_length * sizeof(unsigned long) );

		if ( dante_mca->adc_trace_array == (unsigned long *) NULL ) {
			dante_mca->adc_trace_length = 0;
		...
		dante_mca->adc_trace_length = adc_trace_length;
	} else
	if ( adc_trace_length > dante_mca->adc_trace_length ) {
		dante_mca->adc_trace_array = (unsigned long *)
			realloc( dante_mca->adc_trace_array,
	...
	info[0] = 1.0;
	info[1] = dante_mca->adc_trace_step_size;

	void_ptr = (void *) &(info[0]);

	MX_XIA_SYNC( xiaDoSpecialRun( dante_mca->detector_channel,
					"adc_trace", void_ptr ) );
...
#if 1
	{
		...
		total_delay = 1.0e-9 * dante_mca->adc_trace_step_size
				* (double) dante_mca->adc_trace_length;
	...
	void_ptr = (void *) &(dante_mca->adc_trace_array[0]);
	...
	field->dimension[0] = dante_mca->adc_trace_length;

	return MX_SUCCESSFUL_RESULT;
}

MX_EXPORT mx_status_type
mxd_dante_mca_get_baseline_history_array( MX_MCA *mca )
{
	static const char fname[] =
			"mxd_dante_mca_get_baseline_history_array()";
	...
	if ( dante_mca->baseline_history_array == NULL ) {
	    dante_mca->baseline_history_array = (unsigned long *)
		malloc( baseline_history_length * sizeof(unsigned long) );

	    if ( dante_mca->baseline_history_array == (unsigned long *) NULL)
	    {
		dante_mca->baseline_history_length = 0;
	    ...
	    dante_mca->baseline_history_length = baseline_history_length;
	} else
	if ( baseline_history_length > dante_mca->baseline_history_length ) {
	    dante_mca->baseline_history_array = (unsigned long *)
		realloc( dante_mca->baseline_history_array,
	...
	void_ptr = (void *) &(dante_mca->baseline_history_array[0]);
...
#if 1
	{
		...
		for ( i = 0; i < 100; i++ ) {
			fprintf( stderr, "%lu ",
				dante_mca->baseline_history_array[i] );
		}
	...
	field->dimension[0] = dante_mca->baseline_history_length;
...
#define MXP_DANTE_PARAMETER_NAME_LENGTH	220

/*-------------------------------------------------------------------------*/

MX_EXPORT mx_status_type
mxd_dante_mca_show_parameters( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_show_parameters()";
	...
	MX_XIA_SYNC( xiaGetNumParams( dante_mca->detector_channel,
					&num_parameters ) );
	...
	dimension_array[1] = MXP_DANTE_PARAMETER_NAME_LENGTH+1;
	...
	MX_XIA_SYNC( xiaGetParamData( dante_mca->detector_channel,
	...
	for ( i = 0; i < num_parameters; i++ ) {
		MX_XIA_SYNC( xiaGetParamName( dante_mca->detector_channel,
						i, parameter_name ) );
		...
		snprintf( string_array[i], MXP_DANTE_PARAMETER_NAME_LENGTH,
...
MX_EXPORT mx_status_type
mxd_dante_mca_show_acquisition_values( MX_MCA *mca )
{
	static const char fname[] = "mxd_dante_mca_show_acquisition_values()";
				...
				int operation )
{
	...
	case MX_PROCESS_GET:
		...
		case MXLV_DANTE_MCA_PARAMETER_VALUE:
			...
			mx_status = mxi_dante_read_parameter( mca,
						dante_mca->parameter_name,
						&parameter_value );

			dante_mca->parameter_value = parameter_value;

			MX_DEBUG( 2,
		("%s: Read mca '%s' parameter name = '%s', value = %lu",
				fname, mca->record->name,
				dante_mca->parameter_name,
				dante_mca->parameter_value));
		...
		case MXLV_DANTE_MCA_STATISTICS:
			...
			dante_mca->statistics[ MX_DANTE_MCA_REAL_TIME ]
						= mca->real_time;

			dante_mca->statistics[ MX_DANTE_MCA_LIVE_TIME ]
						= mca->live_time;

			dante_mca->statistics[ MX_DANTE_MCA_ENERGY_LIVE_TIME ]
						= dante_mca->energy_live_time;

			dante_mca->statistics[ MX_DANTE_MCA_NUM_TRIGGERS ]
					= (double) dante_mca->num_triggers;

			dante_mca->statistics[ MX_DANTE_MCA_NUM_EVENTS ]
					= (double) dante_mca->num_events;

			dante_mca->statistics[ MX_DANTE_MCA_INPUT_COUNT_RATE ]
						= dante_mca->input_count_rate;

			dante_mca->statistics[ MX_DANTE_MCA_OUTPUT_COUNT_RATE]
					= dante_mca->output_count_rate;

			dante_mca->statistics[ MX_DANTE_MCA_NUM_UNDERFLOWS ]
					= (double) dante_mca->num_underflows;

			dante_mca->statistics[ MX_DANTE_MCA_NUM_OVERFLOWS ]
					= (double) dante_mca->num_overflows;
		...
		case MXLV_DANTE_MCA_BASELINE_ARRAY:
			...
			mx_status = mxd_dante_mca_get_baseline_array( mca );
			break;
		case MXLV_DANTE_MCA_ACQUISITION_VALUE:
			...
			("%s: get acquisition value '%s' for mca '%s'",
				fname, dante_mca->acquisition_value_name,
				mca->record->name ));
#endif

			mx_status = mxd_dante_mca_get_acquisition_values( mca,
					dante_mca->acquisition_value_name,
					&(dante_mca->acquisition_value) );
		...
		case MXLV_DANTE_MCA_ADC_TRACE_ARRAY:
			...
			mx_status = mxd_dante_mca_get_adc_trace_array( mca );
			break;
		case MXLV_DANTE_MCA_BASELINE_HISTORY_ARRAY:
			...
			mx_status = 
			    mxd_dante_mca_get_baseline_history_array( mca );
			break;
		default:
			MX_DEBUG(-1,(
	...
	case MX_PROCESS_PUT:
		...
		("%s: Write mca '%s' parameter name = '%s', value = %lu",
			...
			mx_status = mxi_dante_write_parameter( mca,
				dante_mca->parameter_name,
				dante_mca->parameter_value );
...
("%s: Write to all channels for mca '%s' parameter name = '%s', value = %lu",
				fname, mca->record->name,
				dante_mca->parameter_name,
				dante_mca->param_value_to_all_channels));

			mx_status = mxi_dante_write_parameter_to_all_channels(
				dante,
				dante_mca->parameter_name,
				dante_mca->param_value_to_all_channels );
		...
		case MXLV_DANTE_MCA_GAIN_CHANGE:
			...
			mx_status = mxd_dante_mca_set_gain_change( mca );
			break;
		case MXLV_DANTE_MCA_GAIN_CALIBRATION:
			...
			("%s: set gain calibration for mca '%s' to %g",
				...
				dante_mca->gain_calibration ));
#endif
			mx_status = mxd_dante_mca_set_gain_calibration( mca );
			break;
		case MXLV_DANTE_MCA_ACQUISITION_VALUE:
			...
			mx_status = mxd_dante_mca_apply( mca );
			break;
		case MXLV_DANTE_MCA_ACQUISITION_VALUE_TO_ALL:
...
#endif
			mx_status =
			    mxi_dante_set_acquisition_values_for_all_channels(
					dante,
					dante_mca->acquisition_value_name,
					&(dante_mca->acquisition_value),
					FALSE );
		...
		case MXLV_DANTE_MCA_SHOW_PARAMETERS:
			...
			MX_DEBUG(-2,("%s: show all parameters for mca '%s'.",
 				fname, mca->record->name));
#endif
			mx_status = mxd_dante_mca_show_parameters( mca );
			break;
		case MXLV_DANTE_MCA_SHOW_ACQUISITION_VALUES:
			...
			("%s: show all acquisition values for mca '%s'.",
 				fname, mca->record->name));
#endif
			mx_status = mxd_dante_mca_show_acquisition_values(mca);
			break;
		default:
			MX_DEBUG( 1,(
