...
("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"));
			...
			default:
				...
				mx_status = mxi_newport_xps_error(
					"GroupMoveAbsolute()",
					newport_xps_motor, NULL,
				newport_xps_motor->move_thread_socket_id,
					xps_status );
				...
				case MXE_WOULD_EXCEED_LIMIT:
					motor->latched_status
					      |= MXSF_MTR_WOULD_EXCEED_LIMIT;
					break;
				case MXE_ILLEGAL_ARGUMENT:
					motor->latched_status
					      |= MXSF_MTR_ILLEGAL_ARGUMENT;
					break;
				default:
					motor->latched_status |= MXSF_MTR_ERROR;
					break;
...
mxd_newport_xps_open( MX_RECORD *record )
{
	...
	/* Initialize the values for the user travel limits. */
	...
	/* Initialize the values for the user travel limits guard bands. */

	mx_status = mx_process_record_field_by_name( record,
						"user_travel_limits_guard_band",
						NULL, MX_PROCESS_GET, NULL );
				...
				int operation )
{
	...
	case MX_PROCESS_GET:
		...
		case MXLV_NEWPORT_XPS_RAW_USER_TRAVEL_LIMITS_GUARD_BAND:
			...
			newport_xps_motor->raw_user_travel_limits_guard_band[0]
			  = fabs( mx_divide_safely( numerator, motor->scale ) );
