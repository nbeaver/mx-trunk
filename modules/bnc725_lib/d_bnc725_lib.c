...
#define MXD_BNC725_LIB_DEBUG	FALSE
...
MX_RECORD_FUNCTION_LIST mxd_bnc725_lib_record_function_list = {
	NULL,
	mxd_bnc725_lib_create_record_structures,
	NULL,
	NULL,
	NULL,
	mxd_bnc725_lib_open
...
MX_PULSE_GENERATOR_FUNCTION_LIST mxd_bnc725_lib_pulse_generator_function_list = {
	mxd_bnc725_lib_busy,
	mxd_bnc725_lib_start,
	mxd_bnc725_lib_stop,
	mxd_bnc725_lib_get_parameter,
	mxd_bnc725_lib_set_parameter
};

MX_RECORD_FIELD_DEFAULTS mxd_bnc725_lib_record_field_defaults[] = {
	MX_RECORD_STANDARD_FIELDS,
	MX_PULSE_GENERATOR_STANDARD_FIELDS,
	MXD_BNC725_LIB_STANDARD_FIELDS
};

long mxd_bnc725_lib_num_record_fields
		= sizeof( mxd_bnc725_lib_record_field_defaults )
			/ sizeof( mxd_bnc725_lib_record_field_defaults[0] );

MX_RECORD_FIELD_DEFAULTS *mxd_bnc725_lib_rfield_def_ptr
			= &mxd_bnc725_lib_record_field_defaults[0];

/* --- */

static mx_status_type
mxd_bnc725_lib_get_pointers( MX_PULSE_GENERATOR *pulse_generator,
			MX_BNC725_LIB_CHANNEL **bnc725_lib_channel,
			MX_BNC725_LIB **bnc725_lib,
			const char *calling_fname )
{
	static const char fname[] = "mxd_bnc725_lib_get_pointers()";

	MX_RECORD *bnc725_lib_channel_record, *bnc725_lib_record;
	MX_BNC725_LIB_CHANNEL *local_bnc725_lib_channel;
	...
	bnc725_lib_channel_record = pulse_generator->record;

	if ( bnc725_lib_channel_record == (MX_RECORD *) NULL ) {
		return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
		"The MX_RECORD pointer for the MX_PULSE_GENERATOR pointer "
		"passed by '%s' is NULL.", calling_fname );
	}

	local_bnc725_lib_channel = (MX_BNC725_LIB_CHANNEL *)
				bnc725_lib_channel_record->record_type_struct;

	if ( bnc725_lib_channel != (MX_BNC725_LIB_CHANNEL **) NULL ) {
		*bnc725_lib_channel = local_bnc725_lib_channel;
	}

	if ( bnc725_lib != (MX_BNC725_LIB **) NULL ) {
		bnc725_lib_record = local_bnc725_lib_channel->bnc725_lib_record;

		if ( bnc725_lib_record == (MX_RECORD *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
			"The bnc725_lib_record pointer for 'bnc725_lib_channel' "
			"record '%s' passed by '%s' is NULL.",
				bnc725_lib_channel_record->name,
				calling_fname );
		}

		*bnc725_lib = (MX_BNC725_LIB *) bnc725_lib_record->record_type_struct;

		if ( *bnc725_lib == (MX_BNC725_LIB *) NULL ) {
			return mx_error( MXE_CORRUPT_DATA_STRUCTURE, fname,
			"The MX_BNC725_LIB pointer for bnc725_lib record '%s' "
			"used by 'bnc725_lib_channel' record '%s' "
			"passed by '%s' is NULL.",
				bnc725_lib_record->name,
				bnc725_lib_channel_record->name,
				calling_fname );
		}
	}
...
mxd_bnc725_lib_create_record_structures( MX_RECORD *record )
{
	static const char fname[] = "mxd_bnc725_lib_create_record_structures()";

	MX_PULSE_GENERATOR *pulse_generator;
	MX_BNC725_LIB_CHANNEL *bnc725_lib_channel = NULL;
	...
	bnc725_lib_channel = (MX_BNC725_LIB_CHANNEL *)
				malloc( sizeof(MX_BNC725_LIB_CHANNEL) );

	if ( bnc725_lib_channel == (MX_BNC725_LIB_CHANNEL *) NULL ) {
		return mx_error( MXE_OUT_OF_MEMORY, fname,
		"Cannott allocate memory for a MX_BNC725_LIB_CHANNEL structure"
		"for record '%s'.", record->name );
	}

	record->record_class_struct = pulse_generator;
	record->record_type_struct = bnc725_lib_channel;

	record->class_specific_function_list =
				&mxd_bnc725_lib_pulse_generator_function_list;

	pulse_generator->record = record;
	bnc725_lib_channel->record = record;

	return MX_SUCCESSFUL_RESULT;
}

MX_EXPORT mx_status_type
mxd_bnc725_lib_open( MX_RECORD *record )
{
	static const char fname[] = "mxd_bnc725_lib_open()";

	MX_PULSE_GENERATOR *pulse_generator;
	MX_BNC725_LIB_CHANNEL *bnc725_lib_channel = NULL;
	MX_BNC725_LIB *bnc725_lib = NULL;
	...
	mx_status = mxd_bnc725_lib_get_pointers( pulse_generator, &bnc725_lib_channel,
						&bnc725_lib, fname );

	if ( mx_status.code != MXE_SUCCESS )
		return mx_status;

#if MXD_BNC725_LIB_DEBUG
	MX_DEBUG(-2,("%s invoked for record '%s'.", fname, record->name));
#endif

	/* Are we configured for a legal channel number? */

	if ( ( bnc725_lib_channel->channel_number < 1 )
	  || ( bnc725_lib_channel->channel_number > bnc725_lib->num_channels ) )
	{
		if ( bnc725_lib->num_channels == 1 ) {
			return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
	"Channel number %ld for 'bnc725_lib_channel' '%s' is illegal.  "
	"The only allowed channel number for 'bnc725_lib' record '%s' is 1.",
				bnc725_lib_channel->channel_number, record->name,
				bnc725_lib->record->name );
		} else {
			return mx_error( MXE_ILLEGAL_ARGUMENT, fname,
	"Channel number %ld for 'bnc725_lib_channel' '%s' is illegal.  "
	"The allowed range of channel numbers for 'bnc725_lib' record '%s' "
	"is from 1 to %ld.",
				bnc725_lib_channel->channel_number, record->name,
				bnc725_lib->record->name, bnc725_lib->num_channels );
		}
	}

	/* Set the pulse train finish time to the current time. */

	if ( bnc725_lib->burst_register_available == FALSE ) {
		bnc725_lib_channel->finish_time = mx_current_clock_tick();
	}
...
MX_EXPORT mx_status_type
mxd_bnc725_lib_busy( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_busy()";
	...
	MX_DEBUG(-2,("%s: record '%s', burst_register_available = %d.",
			...
			bnc725_lib->burst_register_available ));
#endif

	if ( bnc725_lib->burst_register_available ) {

		/* If the burst register is implemented as in firmware
		 * version 3, then we just need to read the BNC725_LIB status
		 * register to see if the pulser is still enabled.
		 */

		mx_status = mx_vme_in32( bnc725_lib->vme_record,
				bnc725_lib->crate_number,
				bnc725_lib->address_mode,
				bnc725_lib->base_address + MX_BNC725_LIB_STATUS_REG,
				&status_register );
...
#if MXD_BNC725_LIB_DEBUG
		MX_DEBUG(-2,("%s: BNC725_LIB '%s' status register = %#lx",
			fname, bnc725_lib->record->name,
			(unsigned long) status_register ));
#endif

		if ( status_register & MXF_BNC725_LIB_STATUS_ENABLE_PULSER_1 ) {
		...
		if ( mx_compare_clock_ticks( current_time,
				bnc725_lib_channel->finish_time ) >= 0 )
		{
			pulse_generator->busy = FALSE;
		} else {
			pulse_generator->busy = TRUE;
	...
	("%s: busy = %d, current_time = (%lu,%lu), finish_time = (%lu,%lu)",
		...
		bnc725_lib_channel->finish_time.high_order,
		(unsigned long) bnc725_lib_channel->finish_time.low_order));
#endif

			/* Stop the pulse train. */

			mx_status = mxd_bnc725_lib_stop( pulse_generator );
...
static mx_status_type
mxd_bnc725_lib_send_single_pulse( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_send_single_pulse()";
	...
	pulse_register = (uint32_t) bnc725_lib->base_address;

	switch( bnc725_lib_channel->channel_number ) {
	case 1:
		pulse_register += MX_BNC725_LIB_PULSE_CHANNEL_1;
		break;
	case 2:
		pulse_register += MX_BNC725_LIB_PULSE_CHANNEL_2;
		break;
	case 3:
		pulse_register += MX_BNC725_LIB_PULSE_CHANNEL_3;
		break;
	case 4:
		pulse_register += MX_BNC725_LIB_PULSE_CHANNEL_4;
		break;
	}

	/* Send the pulse. */

	mx_status = mx_vme_out32( bnc725_lib->vme_record,
					bnc725_lib->crate_number,
					bnc725_lib->address_mode,
					pulse_register,
					1 );
...
static mx_status_type
mxd_bnc725_lib_start_with_burst_register( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_start_with_burst_register()";
	...
	/* Since the BNC725_LIB has special hardware for generating a
	 * single pulse, we must treat pulse_generator->num_pulses == 1
	 * as a special case.
	 */

	if ( pulse_generator->num_pulses == 1 ) {
		mx_status = mxd_bnc725_lib_send_single_pulse( pulse_generator );
	...
	enable_channel_value = 1 << ( 3 + bnc725_lib_channel->channel_number );
	...
	mx_status = mx_vme_out32( bnc725_lib->vme_record,
				bnc725_lib->crate_number,
				bnc725_lib->address_mode,
				bnc725_lib->base_address + MX_BNC725_LIB_CONTROL_REG,
...
static mx_status_type
mxd_bnc725_lib_start_without_burst_register( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[]
		= "mxd_bnc725_lib_start_without_burst_register()";
	...
	if ( pulse_generator->num_pulses == 1 ) {
		...
		if ( pulse_generator->mode != MXF_PGN_PULSE ) {
			bnc725_lib_channel->finish_time = start_time;
		} else {
			pulse_time = 80.0e-9;
			...
			bnc725_lib_channel->finish_time = mx_add_clock_ticks(
	...
	bnc725_lib_channel->finish_time = finish_time;

	return MX_SUCCESSFUL_RESULT;
}

MX_EXPORT mx_status_type
mxd_bnc725_lib_start( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_start()";
	...
	if ( bnc725_lib->burst_register_available ) {
		mx_status =
		    mxd_bnc725_lib_start_with_burst_register( pulse_generator );
	} else {
		mx_status =
		    mxd_bnc725_lib_start_without_burst_register( pulse_generator );
	}
...
MX_EXPORT mx_status_type
mxd_bnc725_lib_stop( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_stop()";
	...
	disable_channel_value = 1 << ( 11 + bnc725_lib_channel->channel_number );
...
MX_EXPORT mx_status_type
mxd_bnc725_lib_get_parameter( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_get_parameter()";
	...
	case MXLV_PGN_PULSE_WIDTH:
		...
		if ( bnc725_lib->pulse_width_available == FALSE ) {
			if ( pulse_generator->mode == MXF_PGN_PULSE ) {
				if ( pulse_generator->num_pulses == 1 ) {
	...
	case MXLV_PGN_PULSE_DELAY:
		...
		/* Pulse delays are not supported for the BNC725_LIB, so
		 * always return 0.
		 */
		...
		break;
	case MXLV_PGN_MODE:
		/* Read the BNC725_LIB status register. */
		...
		mask = 1 << ( 15 + bnc725_lib_channel->channel_number );
...
MX_EXPORT mx_status_type
mxd_bnc725_lib_set_parameter( MX_PULSE_GENERATOR *pulse_generator )
{
	static const char fname[] = "mxd_bnc725_lib_set_parameter()";
 ...
 *              the BNC725_LIB is a runt pulse with an extremely narrow width
 ...
 *              to 0 or 1 have special meanings for the BNC725_LIB, so the
 *              extra pulse is only added for the case of num_pulses > 1.
 */
			flags = bnc725_lib_channel->bnc725_lib_flags;

			if ( flags & MXF_BNC725_LIB_RUNT_PULSE_FIX ) {
				if ( num_pulses > 1 ) {
					num_pulses++;
				}
			}
			...
			mx_status = mx_vme_out32( bnc725_lib->vme_record,
					bnc725_lib->crate_number,
					bnc725_lib->address_mode,
			bnc725_lib->base_address + MX_BNC725_LIB_BURST_REGISTER,
					num_pulses );
	...
	case MXLV_PGN_MODE:
		...
		case MXF_PGN_PULSE:
			control_register =
				1 << ( 23 + bnc725_lib_channel->channel_number );
			break;
		case MXF_PGN_SQUARE_WAVE:
			control_register =
				1 << ( 15 + bnc725_lib_channel->channel_number );
			break;
		default:
		...
		break;
	case MXLV_PGN_PULSE_WIDTH:
		if ( bnc725_lib->pulse_width_available ) {
		...
		"than the maximum allowed pulse width %g.",
	      ...
	      PULSE_WIDTH_STEP_SIZE * (double) TWO_TO_THE_8TH_POWER_MINUS_ONE );
			...
			mx_status = mx_vme_out32( bnc725_lib->vme_record,
						bnc725_lib->crate_number,
						bnc725_lib->address_mode,
				bnc725_lib->base_address + MX_BNC725_LIB_PULSE_WIDTH,
						pulse_width_register );
			...
			/* This version of the BNC725_LIB firmware does not
	...
	case MXLV_PGN_PULSE_PERIOD:
		...
		if ( bnc725_lib->enable_factor_8_predivider ) {
		...
		if ( preset_value_plus_one >= MX_BNC725_LIB_MAXIMUM_PRESET ) {
			return mx_error( MXE_WOULD_EXCEED_LIMIT, fname,
			"The pulse period (%g seconds) requested for "
			...
			"maximum period of %g seconds.",
				...
				pulse_generator->record->name,
				time_quantum * (double)
					( 1 + MX_BNC725_LIB_MAXIMUM_PRESET ) );
		}
		...
		preset_register = (uint32_t) bnc725_lib->base_address;

		switch( bnc725_lib_channel->channel_number ) {
		case 1:
			preset_register += MX_BNC725_LIB_PRESET_FACTOR_PULSER_1;
			break;
		case 2:
			preset_register += MX_BNC725_LIB_PRESET_FACTOR_PULSER_2;
			break;
		case 3:
			preset_register += MX_BNC725_LIB_PRESET_FACTOR_PULSER_3;
			break;
		case 4:
			preset_register += MX_BNC725_LIB_PRESET_FACTOR_PULSER_4;
			break;
		}
